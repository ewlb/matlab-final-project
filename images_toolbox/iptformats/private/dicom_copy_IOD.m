...
if(~isempty(X))
    metadata = images.internal.dicom.dicom_prep_ImagePixel(metadata, X, map, options.txfr, options.usemetadatabitdepths, dictionary);
else
    PixelAtr = {'SamplesPerPixel','PhotometricInterpretation',...
        'Rows','Columns' 'BitsAllocated','BitsStored','HighBit',...
        'PixelRepresentation','SmallestImagePixelValue', 'LargestImagePixelValue'};
    
    N = numel(PixelAtr);
    
    %For empty image, remove pixel related attributes from metadata
    for i = 1:N
        if(isfield(metadata,PixelAtr(i)))
            metadata = rmfield(metadata,PixelAtr(i));
        end
    end
end
...
for p = 1:numel(fields_to_write)
    ...
    new_attr = images.internal.dicom.dicom_convert_meta_to_attr(attr_name, metadata, dictionary, options.txfr, specificCharacterSet, []);
