...
[compData, hasAPP6] = getJpegStream(details);
...
if hasAPP6
    ...
    nColsBlocks = ceil(details.imageHeight / details.tileHeight);
    nRowsBlocks = ceil(details.imageWidth / details.tileWidth);
end
...
persistent jpegReader
if isempty(jpegReader)
    jpegFormatFcns = imformats('jpeg');
    jpegReader = jpegFormatFcns.read;
end
...
for j = 1:nRowsBlocks
    ...
    for i = 1:nColsBlocks
        ...
        data = getImageDataFromFrame(name, frameData, jpegReader);
...
function [compData, hasAPP6] = getJpegStream(details)
...
hasAPP6 = isequal(compData(3:4), [255; 230]);
if ~hasAPP6
    warning(message('images:nitfread:missingAppMarker'))
end
...
function img = getImageDataFromFrame(name, frameData, jpegReader)
