...
%   Copyright 2017-2020 The MathWorks, Inc.
    ...
    methods 
        ...
        %    the object. It returns the image as a numeric vector 'img'.
            ...
            if fid > 0
                ...
                % First index indicates num of valid dimensions in the file
                numValidDims = headerStruct.dim(1);
                imageSize = prod(headerStruct.dim(2:numValidDims+1));
                ...
                % remove squeeze.
                img = (reshape(img, headerStruct.dim(2:numValidDims+1)));
                if isempty(img)
        ...
        %    form.
            ...
            %Set Version as 'NIfTI1' or 'NIfTI2'
            if rawStruct.sizeof_hdr==348
                simplifiedStruct.Version = 'NIfTI1';
            elseif rawStruct.sizeof_hdr==540
                simplifiedStruct.Version = 'NIfTI2';
            end
        ...
        function [fid, bytesWritten] = writeHeader(self, fid, machineFmt)
            ...
            if (fid > 0)
                if headerStruct.sizeof_hdr == 348
                    ...
                    count = fwrite(fid, headerStruct.sizeof_hdr, 'int32', machineFmt);
                    bytesWritten = bytesWritten + count*4; % int32 = 4 bytes.
                    count = fwrite(fid, headerStruct.magic, 'char', machineFmt);
                    bytesWritten = bytesWritten + count; % 8*1 bytes.
                    count = fwrite(fid, headerStruct.datatype, 'short', machineFmt);
                    bytesWritten = bytesWritten + count*2; % 2 bytes.
                    count = fwrite(fid, headerStruct.bitpix, 'short', machineFmt);
                    bytesWritten = bytesWritten + count*2; % 2 bytes.
                    count = fwrite(fid, headerStruct.dim, 'int64', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8*8 bytes.
                    count = fwrite(fid, headerStruct.intent_p1, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.intent_p2, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.intent_p3, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.pixdim, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8*8 bytes.
                    count = fwrite(fid, headerStruct.vox_offset, 'int64', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.scl_slope, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.scl_inter, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.cal_max, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.cal_min, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.slice_duration, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.toffset, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.slice_start, 'int64', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.slice_end, 'int64', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    ...
                    count = fwrite(fid, headerStruct.qform_code, 'int32', machineFmt);
                    bytesWritten = bytesWritten + count*4; % 4 bytes.
                    count = fwrite(fid, headerStruct.sform_code, 'int32', machineFmt);
                    bytesWritten = bytesWritten + count*4; % 4 bytes.
                    
                    count = fwrite(fid, headerStruct.quatern_b, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.quatern_c, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.quatern_d, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    
                    count = fwrite(fid, headerStruct.qoffset_x, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.qoffset_y, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    count = fwrite(fid, headerStruct.qoffset_z, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 8 bytes.
                    
                    count = fwrite(fid, headerStruct.srow_x, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 4*8 bytes.
                    count = fwrite(fid, headerStruct.srow_y, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 4*8 bytes.
                    count = fwrite(fid, headerStruct.srow_z, 'double', machineFmt);
                    bytesWritten = bytesWritten + count*8; % 4*8 bytes.
                    
                    count = fwrite(fid, headerStruct.slice_code, 'int32', machineFmt);
                    bytesWritten = bytesWritten + count*4; % 4 byte.
                    count = fwrite(fid, headerStruct.xyzt_units, 'int32', machineFmt);
                    bytesWritten = bytesWritten + count*4; % 4 byte.
                    count = fwrite(fid, headerStruct.intent_code, 'int32', machineFmt);
                    bytesWritten = bytesWritten + count*4; % 4 bytes.
                    ...
                    count = fwrite(fid, repmat(' ', 1, 15), 'uchar', machineFmt);%unused_str
                    bytesWritten = bytesWritten + count; % 15 bytes.
                end
            else
            ...
            try
                if self.header.sform_code > 0
                    ...
                    xform = affine3d([self.header.srow_x; ...
                                      ...
                                      self.header.srow_z; ...
                                      0 0 0 1]');
                elseif (self.header.qform_code > 0) || (self.header.qform_code == -1)
                    xformName = 'Qform';
                    ...
                    if (self.header.qform_code == -1)
                        % Flip 3rd column for improper transformation
                        k = k*self.header.qform_code;
                    end
    ...
    methods(Static, Access = public, Hidden)
        ...
        %    'little' or 'big', which is then used to read the volume data.
            ...
            if position < 348
               fclose(fid);
               error(message('images:nifti:headerWrongSize'));
            end
            ...
            if fid > 0
                ...
                if headerStruct.sizeof_hdr ~= 348 && headerStruct.sizeof_hdr ~= 540
                    fclose(fid);
                ...
                if headerStruct.sizeof_hdr == 348
                    ...
                    %NIfTI2 
                    headerStruct.magic = fread(fid, 8, 'uchar=>char')';
                    headerStruct.datatype = fread(fid, 1, 'short');
                    headerStruct.bitpix = fread(fid, 1, 'short');
                    headerStruct.dim = fread(fid, 8, 'int64')';
                    headerStruct.intent_p1 = fread(fid, 1, 'double');
                    headerStruct.intent_p2 = fread(fid, 1, 'double');
                    headerStruct.intent_p3 = fread(fid, 1, 'double');
                    headerStruct.pixdim = fread(fid, 8, 'double')';
                    headerStruct.vox_offset = fread(fid, 1, 'int64');
                    headerStruct.scl_slope = fread(fid, 1, 'double');
                    headerStruct.scl_inter = fread(fid, 1, 'double');
                    headerStruct.cal_max = fread(fid, 1, 'double');
                    headerStruct.cal_min = fread(fid, 1, 'double');
                    headerStruct.slice_duration = fread(fid, 1, 'double');
                    headerStruct.toffset = fread(fid, 1, 'double');
                    headerStruct.slice_start = fread(fid, 1, 'int64');
                    headerStruct.slice_end = fread(fid, 1, 'int64');
                    headerStruct.descrip = deblank(fread(fid, 80, 'uchar=>char')');
                    headerStruct.aux_file = deblank(fread(fid, 24, 'uchar=>char')');
                    
                    headerStruct.qform_code = fread(fid, 1, 'int32');%%
                    headerStruct.sform_code = fread(fid, 1, 'int32');
                    
                    headerStruct.quatern_b = fread(fid, 1, 'double');
                    headerStruct.quatern_c = fread(fid, 1, 'double');
                    headerStruct.quatern_d = fread(fid, 1, 'double');
                    
                    headerStruct.qoffset_x = fread(fid, 1, 'double');
                    headerStruct.qoffset_y = fread(fid, 1, 'double');
                    headerStruct.qoffset_z = fread(fid, 1, 'double');
                    
                    headerStruct.srow_x = fread(fid, 4, 'double')';
                    headerStruct.srow_y = fread(fid, 4, 'double')';
                    headerStruct.srow_z = fread(fid, 4, 'double')';
                    
                    headerStruct.slice_code = fread(fid, 1, 'int32'); 
                    headerStruct.xyzt_units = fread(fid, 1, 'int32');
                    headerStruct.intent_code = fread(fid, 1, 'int32');
                    headerStruct.intent_name = deblank(fread(fid, 16, 'uchar=>char')');
                    headerStruct.dim_info = fread(fid, 1, 'uchar=>char')';
                    fread(fid, 15, 'int32'); % unused_str (unused)
                end
        ...
        function headerStruct = niftiDefaultHeader(V, isNII, Version)
        ...
        %    0/empty/unknown.
            if strcmp(Version, 'NIfTI1')
                ...
                %NIfTI 2
                headerStruct.sizeof_hdr = 540;
                
                if isNII
                    headerStruct.magic = sprintf('n+2\0\0\0\0\0'); % .nii
                else
                    headerStruct.magic = sprintf('ni1\0\0\0\0\0'); % .img/.hdr
                end
                [headerStruct.datatype, headerStruct.bitpix] = images.internal.nifti.niftiImage.setDataType(class(V));
                imageDims = length(size(V));
                headerStruct.dim = [imageDims size(V) ones(1, 7 - imageDims)];
                ...
                headerStruct.slice_code = 0;
                headerStruct.xyzt_units = images.internal.nifti.niftiImage.setSpaceTimeUnits('Unknown', 'None');
                headerStruct.intent_code = intent_code;
                headerStruct.intent_name = intent_name;
                headerStruct.dim_info = ' ';
                
            end
        end
        
        function rawStruct = toRawStruct(simpleStruct, isNII, Version)
            ...
            if strcmp(Version, 'NIfTI1')
                ...
                if isfield(simpleStruct, 'TransformName') && isfield(simpleStruct, 'Transform')
                    [rawStruct, simpleStruct] = images.internal.nifti.niftiImage.setXForm(rawStruct, simpleStruct);
                    rawStruct.pixdim(1) = simpleStruct.Qfactor;
                else
                ...
                % NIfTI2
                rawStruct.sizeof_hdr = 540;
                if isNII
                    rawStruct.magic = sprintf('n+2\0\0\0\0\0');
                else
                    rawStruct.magic = sprintf('ni1\0\0\0\0\0');
                end
                [rawStruct.datatype, rawStruct.bitpix] = images.internal.nifti.niftiImage.setDataType(simpleStruct.Datatype);
                imageDims = length(simpleStruct.ImageSize);
                rawStruct.dim = [imageDims simpleStruct.ImageSize ones(1, 7 - imageDims)];
                ...
                rawStruct.slice_code = images.internal.nifti.niftiImage.setSliceCode(simpleStruct.SliceCode);
                rawStruct.xyzt_units = images.internal.nifti.niftiImage.setSpaceTimeUnits(simpleStruct.SpaceUnits, simpleStruct.TimeUnits);
                rawStruct.intent_code = intent_code;
                rawStruct.intent_name = intent_name;
                rawStruct.dim_info = images.internal.nifti.niftiImage.setDimInfo(simpleStruct.FrequencyDimension, simpleStruct.PhaseDimension, simpleStruct.SpatialDimension);
    ...
    methods(Static, Access = private)
        ...
        function [rawStruct, simpleStruct] = setXForm(rawStruct, simpleStruct)
        ...
        %    specified by the NIfTI standard.
            ...
            elseif strcmp(simpleStruct.TransformName,'Qform')
                ...
                % Take out rotation matrix
                R = simpleStruct.Transform.T(1:3,1:3);
                
                % Converting to orthogonal matrix using polar decomposition
                [W,~,V] = svd(R');
                R = W*V';
                        
                % Compute the determinant to determine if it is improper.
                % Negate the third column for improper transformation.
                zd = det(R);
                if zd < 1e-5
                    simpleStruct.Qfactor = -1;   % For improper transformation the first value of pixdim is -1.
                    rawStruct.pixdim(1) = -1;    
                    R(:,3) = -R(:,3);
                end
                ...
                if a>0.5
                    a = 0.5 * sqrt(a);
                    rawStruct.quatern_b = 0.25 * (R(3,2)-R(2,3))/a;
                    rawStruct.quatern_c = 0.25 * (R(1,3)-R(3,1))/a;
                    rawStruct.quatern_d = 0.25 * (R(2,1)-R(1,2))/a;
                    ...
                    if xd>1
                        rawStruct.quatern_b = 0.5 * sqrt(xd);
                        rawStruct.quatern_c = 0.25 * (R(1,2)+R(2,1))/rawStruct.quatern_b;
                        rawStruct.quatern_d = 0.25 * (R(1,3)+R(3,1))/rawStruct.quatern_b;
                        a = 0.25 * (R(3,2)-R(2,3))/rawStruct.quatern_b;
                    elseif yd>1
                        rawStruct.quatern_c = 0.5 * sqrt(yd) ;
                        rawStruct.quatern_b = 0.25 * (R(1,2)+R(2,1)) / rawStruct.quatern_c ;
                        rawStruct.quatern_d = 0.25 * (R(2,3)+R(3,2)) / rawStruct.quatern_c ;
                        a = 0.25 * (R(1,3)-R(3,1)) / rawStruct.quatern_c ;
                    else
                        rawStruct.quatern_d = 0.5 * sqrt(zd) ;
                        rawStruct.quatern_b = 0.25 * (R(1,3)+R(3,1)) / rawStruct.quatern_d ;
                        rawStruct.quatern_c = 0.25 * (R(2,3)+R(3,2)) / rawStruct.quatern_d ;
                        a = 0.25 * (R(2,1)-R(1,2)) / rawStruct.quatern_d ;
                    end
                    if a<0
                        rawStruct.quatern_b = -rawStruct.quatern_b;
                        rawStruct.quatern_c = -rawStruct.quatern_c;
                        rawStruct.quatern_d = -rawStruct.quatern_d;
                    end
                end
