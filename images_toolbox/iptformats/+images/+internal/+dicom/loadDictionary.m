function [tags, values] = loadDictionary(dictionary)
%loadDictionary  Load the DICOM data dictionary into MATLAB
...
%
fmt = '(%4c,%4c)\t%s%s%s';                                     
fileID = fopen(dictionary);
if (fileID < 0)
    error(message('images:dicomdict:fileNotFound', dictionary));
end
C = textscan(fileID, fmt, 'CommentStyle','#');
[group, element, vr, name, vm] = C{:}; 
fclose(fileID);
...
% from the data dictionary which have been superseded by later entries.
...
for p = 1:length(vr)
    ...
    if (~isempty(strfind(vr{p}, '/')))
        % Multiple potential VR's.
        
        tmp_vr = images.internal.dicom.tokenize(vr{p}, '/');
        
        for q = 1:length(tmp_vr)
            if (length(tmp_vr{q}) ~= 2)
                error(message('images:dicom_load_dictionary:invalidVR', vr{ p }, group( p, : ), element( p, : )))
            end
        end
    ...
    if (~isempty(h_pos))
        ...
        if ((length(h_pos) > 1) || ...
            ...
            error(message('images:dicom_load_dictionary:invalidVM', vm{ p }, group( p, : ), element( p, : )))
        end
...
rows = zeros(total_attr, 1);
cols = zeros(total_attr, 1);
