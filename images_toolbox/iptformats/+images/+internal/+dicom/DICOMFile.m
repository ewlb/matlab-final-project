classdef DICOMFile < handle
    ...
    methods
        function obj = DICOMFile(filename)
            ...
            readPixelData = false;
            readAllMeta = true;
            obj.RawAttrs = images.internal.builtins.dicomparse(fileDetails.name, ...
                fileDetails.bytes, ...
                getMachineEndian(), ...
                readPixelData, ...
                obj.Dictionary, ...
...
function set.PixelData(obj, value)
    ...
    methods (Access = private)
            ...
            switch (rawAttr.VR)
                ...
                case 'PN'
                    if topLevel
                        ...
                        specificCharacterSet = images.internal.dicom.getSpecificCharacterSet(obj);
                    else
                        % Get SpecificCharacterSet from sibling metadata currently being parsed.
                        specificCharacterSet = images.internal.dicom.getSpecificCharacterSet(siblingMetadata, obj.Dictionary);
                    end
                    processedAttr = images.internal.dicom.localizePN(rawAttr.Data, specificCharacterSet);
        ...
        function [vr, name] = findVRFromTag(obj, group, element)
            ...
            attr = images.internal.builtins.dicomlookup_helper(group, element, obj.Dictionary);
...
function addRawAttribute(obj, group, element, value)
            ...
            dictionaryAttr = images.internal.builtins.dicomlookup_helper(group, element, obj.Dictionary);
...
if ((isequal(uidDetails.VR, 'Implicit')) && (attr.Group > 2))
    ...
    switch (attr.VR)
        ...
        otherwise
            % PS 3.5-1999 Sec. 6.2 indicates that all unknown VRs can be
            % interpreted as being the same as OB, OW, SQ, or UN.  The
