...
pixel_cells = images.internal.dicom.typecast(pixel_cells, 'uint8', swap);
pixel_cells = reshape(pixel_cells, [double(num_segments), ...
                                    (numel(pixel_cells) / double(num_segments))]);
...
for f = 1:numFrames
    ...
    for p = 1:num_segments
        ...
        for q = 1:dims(1)
            ...
            encoded_row = images.internal.builtins.dicom_encode_rle_segment(pixel_cells(p, start_idx:end_idx));
    ...
    encoded_data = [images.internal.dicom.typecast(header, 'uint8', swap); encoded_data]; %#ok<AGROW>
