function attr = dicom_convert_meta_to_attr(attr_name, metadata, dictionary, txfr, specificCharacterSet, VR)
...
if (isequal(VR, 'PN') || isPersonName(metadata.(attr_name)))
    ...
    data = images.internal.dicom.dicom_encode_pn(metadata.(attr_name), specificCharacterSet);
...
% Add the attribute.
if (isempty(VR))
    attr = images.internal.dicom.dicom_add_attr([], tag(1), tag(2), dictionary, specificCharacterSet, data);
else
    attr = images.internal.dicom.dicom_add_attr([], tag(1), tag(2), dictionary, specificCharacterSet, data, VR);
end
...
attr_details = images.internal.dicom.dicom_dict_lookup(tag(1), tag(2), dictionary);
...
if (isempty(attr_details))
    ...
    if (iscell(VR))
        ...
        elseif (isequal(tag, uint16([sscanf('7fe0', '%x'), ...
            ...
            uidDetails = images.internal.dicom.dicom_uid_decode(txfr);
...
for p = 1:numel(items)
    ...
    attrs = images.internal.dicom.dicom_add_attr(attrs, 'fffe', 'e000', dictionary, specificCharacterSet, data);
...
% use it locally.
SCSFromItem = images.internal.dicom.dicom_get_SpecificCharacterSet(item_struct, dictionary);
...
for p = 1:numel(attr_names)
    ...
    new_attr = images.internal.dicom.dicom_convert_meta_to_attr(attr_names{p}, item_struct, dictionary, txfr, specificCharacterSet, []);
