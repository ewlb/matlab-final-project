...
metadata(1).(images.internal.dicom.lookupActions('0028', '0002', dictionary)) = size(X, 3);
metadata.(images.internal.dicom.lookupActions('0028', '0004', dictionary)) = getPhotometricInterp(metadata, X, map, txfr, dictionary);
metadata.(images.internal.dicom.lookupActions('0028', '0010', dictionary)) = size(X, 1);
metadata.(images.internal.dicom.lookupActions('0028', '0011', dictionary)) = size(X, 2);

[ba, bs, hb, pr] = getPixelStorage(X, txfr, useExistingBitDepths, metadata, dictionary);
metadata.(images.internal.dicom.lookupActions('0028', '0100', dictionary)) = ba;
metadata.(images.internal.dicom.lookupActions('0028', '0101', dictionary)) = bs;
metadata.(images.internal.dicom.lookupActions('0028', '0102', dictionary)) = hb;
metadata.(images.internal.dicom.lookupActions('0028', '0103', dictionary)) = pr;

metadata.(images.internal.dicom.lookupActions('7FE0', '0010', dictionary)) = encodePixelData(metadata, X, map, txfr, dictionary);

% Values that depend on the number of samples present.
if (metadata.(images.internal.dicom.lookupActions('0028', '0002', dictionary)) > 1)

    metadata.(images.internal.dicom.lookupActions('0028', '0006', dictionary)) = 0;  % Interleaved pixels.
    ...
    switch class(X)
        case {'single', 'double', 'logical'}
            ...
            data = metadata.(images.internal.dicom.lookupActions('7FE0', '0010', dictionary));
            metadata.(images.internal.dicom.lookupActions('0028', '0106', dictionary)) = min(data(:));
            metadata.(images.internal.dicom.lookupActions('0028', '0107', dictionary)) = max(data(:));
        otherwise
            metadata.(images.internal.dicom.lookupActions('0028', '0106', dictionary)) = min(X(:));
            metadata.(images.internal.dicom.lookupActions('0028', '0107', dictionary)) = max(X(:));
    end
...
if (~isempty(map))
    ...
    metadata.(images.internal.dicom.lookupActions('0028', '1101', dictionary)) = rdes;
    metadata.(images.internal.dicom.lookupActions('0028', '1102', dictionary)) = gdes;
    metadata.(images.internal.dicom.lookupActions('0028', '1103', dictionary)) = bdes;
    metadata.(images.internal.dicom.lookupActions('0028', '1201', dictionary)) = rdata;
    metadata.(images.internal.dicom.lookupActions('0028', '1202', dictionary)) = gdata;
    metadata.(images.internal.dicom.lookupActions('0028', '1203', dictionary)) = bdata;
end
...
% copying metadata.
if ((isfield(metadata, images.internal.dicom.lookupActions('0028', '0008', dictionary))) || ...
    (size(X,4) > 1))
  
    % Add the attribute if it isn't present and there's more than one frame.
    metadata.(images.internal.dicom.lookupActions('0028', '0008', dictionary)) = size(X, 4);
...
if (isempty(map))
    ...
    if (size(X, 3) == 1)
        ...
        pIntFieldname = images.internal.dicom.lookupActions('0028', '0004', dictionary);
...
if (useExistingBitDepths)
    % BitsAllocated
    if isfield(metadata, images.internal.dicom.lookupActions('0028', '0100', dictionary))
        ba = metadata.(images.internal.dicom.lookupActions('0028', '0100', dictionary));
    end
    
    % BitsStored
    if isfield(metadata, images.internal.dicom.lookupActions('0028', '0101', dictionary))
        bs = metadata.(images.internal.dicom.lookupActions('0028', '0101', dictionary));
    end
    
    % HighBit
    if isfield(metadata, images.internal.dicom.lookupActions('0028', '0102', dictionary))
        hb = metadata.(images.internal.dicom.lookupActions('0028', '0102', dictionary));
    end
end
...
if (bs > ba)
    ...
    error(message('images:dicom_prep_ImagePixel:bitsStoredTooBig', ...
                  images.internal.dicom.lookupActions('0028', '0101', dictionary), ...
                  images.internal.dicom.lookupActions('0028', '0100', dictionary)))

elseif (hb >= ba)

    error(message('images:dicom_prep_ImagePixel:highBitTooBig', ...
                  images.internal.dicom.lookupActions('0028', '0102', dictionary), ...
                  images.internal.dicom.lookupActions('0028', '0100', dictionary)))
...
otherwise
    ...
    if strcmp(txfr,'1.2.840.10008.1.2.5')&&~islogical(X)
        X = swapbytes(X);
    end
    pixelCells = images.internal.dicom.dicom_encode_pixel_cells(X, map, ...
        metadata.(images.internal.dicom.lookupActions('0028', '0100', dictionary)), ...
        metadata.(images.internal.dicom.lookupActions('0028', '0101', dictionary)), ...
        metadata.(images.internal.dicom.lookupActions('0028', '0102', dictionary)));
end

%
% Encode pixels.
%
uid_details = images.internal.dicom.dicom_uid_decode(txfr);
if (uid_details.Compressed)
    
    % Compress the pixel cells and add delimiters.
    pixelData = images.internal.dicom.dicom_compress_pixel_cells(pixelCells, txfr, ...
                                    metadata.(images.internal.dicom.lookupActions('0028','0100', dictionary)), ...
                                    size(X));
    specificCharacterSet = {};  % Not needed for encoding
    pixelData = images.internal.dicom.dicom_encode_attrs(pixelData, txfr, images.internal.dicom.dicom_uid_decode(txfr), specificCharacterSet);
...
if (~isequal(uid_details.Endian, uid_details.PixelEndian))
    ...
    if (metadata.(images.internal.dicom.lookupActions('0028', '0100', dictionary)) <= 16)  % BitsAllocated
    ...
    if (isequal(endian, 'B') && ...
        ...
        metadata.(images.internal.dicom.lookupActions('0028', '0100', dictionary)) > 16)
