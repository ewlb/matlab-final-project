...
% If it's a sequence, recursively encode the items and attributes.
if (isstruct(attr.Data))
    ...
    if ((isequal(attr.VR, 'PN')) || ...
        ...
        attr.Data = images.internal.dicom.dicom_encode_pn(attr.Data, specificCharacterSet);
...
if ((isequal(uid_details.VR, 'Implicit')) && (attr.Group > 2))
    ...
    case {'AE','AS','AT','CS','DA','DS','DT','FD','FL','IS', ...
          'LO','LT', 'PN','SH','SL','SS','ST','TM','UI','UL','US','UC','UR'} 
        
        len = data_size * length(attr.Data);
        if len>=65534
            % https://dicom.nema.org/dicom/2013/output/chtml/part05/sect_6.2.html
            % 65534 is the upper limit
            warning(message('images:dicomwrite:exceedsLengthLimit',dicomlookup(attr.Group, attr.Element), len));
            attr.Data = attr.Data(1:65534);
            len = 65534;
        end
        len = uint16(len);
    ...
    otherwise
        ...
        % be interpreted as being the same as OB, OW, SQ, or UN.  The
...
for p = 1:numel(struct_data)
    ...
    tmp = images.internal.dicom.dicom_encode_attrs(struct_data(p), uid_details.Value, uid_details, specificCharacterSet);
...
icuCharacterSet = images.internal.dicom.dicom_getConverterString(specificCharacterSet);
