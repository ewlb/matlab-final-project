classdef CollectionLoader < handle
    ...
    methods (Access = private)
        ...
        function describeSource(obj, source, includeSubdir)
            ...
            if ~isfolder(source)  % Use isfolder() instead of isfile() to support parial directory names
                fid = fopen(source, 'r');
                source = fopen(fid);
                fclose(fid);
        ...
        function fileDetails = getFileDetails(obj, dirName, details)
            % Go through every file in the directory, make a DICOMFile for
            % it, and get all of the metadata we care about for the
            % collection. If it's not a DICOM file, just skip it.
            ...
            for idx = 1:numel(details)
                ...
                try
                    ...
                    fileDetails(currentIndex).DICOMFiles = thisFile;  % Hold onto this to prevent re-parsing later.
                catch
...
for idx = uniqueIndices
    ...
    seriesDetails(outputIndex).DICOMFiles = cat(1, filesToAggregate(partOfTheSeries).DICOMFiles);
...
SOPClassUID = fileObject.getAttribute(2,2);  % MediaStorageSOPClassUID
if ~isempty(SOPClassUID)
    switch SOPClassUID
