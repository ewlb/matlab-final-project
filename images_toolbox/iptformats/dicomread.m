...
readPixels = true;
[attrs, settings] = images.internal.builtins.dicomparse(fileDetails.name, ...
                               fileDetails.bytes, ...
                               machineEndian, ...
                               readPixels, ...
                               dictionaryFile, ...
...
if ~txfrDetails.EncodingSupported
    ...
    error(message('images:dicomread:unsupportedEncoding', metadata.TransferSyntaxUID))
...
% character in the (0028,0008) attribute or a uint16 value in (0054,0081).
...
d = {'0002', '0010', 'TransferSyntaxUID',         'char',   ''
     ...
     '0054', '0081', 'NumberOfSlices',            'uint16', []
...
elseif (~isempty(metadata.NumberOfSlices))

    nFrames = metadata.NumberOfSlices;
...
% We store any variable that is not going to change between multiple
...
switch (dictionaryEntry.Datatype)
    ...
    case 'uint16'
        ...
        if rem(numel(attr.Data),2)
            error(message("images:dicomread:incorrectSize", dictionaryEntry.Name));
        end
        if (swapAttrBytes)
...
if (isempty(UID))
    ...
    switch (UID)
        ...
        case {'1.2.840.10008.1.2.4.100'
            ...
            details.EncodingSupported = true;
            details.DataCompressed = true;
            details.DecompressFcn = @decompressVideo;
            details.RequiresColorspaceConversion = false;
...
% Check if BitsAllocated and BitsStored are both empty or zero. If one of
% them is missing, 0 or empty, pick its value from the  other. But if both
% are missing/zero/empty, error out.
isBitsAllocatedValid = isfield(metadata,'BitsAllocated')...
    && ~isempty(metadata.BitsAllocated) ...
    && metadata.BitsAllocated ~= 0;
isBitsStoredValid = isfield(metadata,'BitsStored')...
    && ~isempty(metadata.BitsStored) ...
    && metadata.BitsStored ~= 0;

if isBitsAllocatedValid && ~isBitsStoredValid
    metadata.BitsStored = metadata.BitsAllocated;
elseif isBitsStoredValid && ~isBitsAllocatedValid
...
elseif (metadata.BitsAllocated == 8)
    ...
    % Unpack the data.
    metadata.InstanceData = images.internal.builtins.bitparse(metadata.InstanceData, ...
    ...
    if (metadata.PixelRepresentation == 1)
        ...
        % the sign bit position and then shifting back. This propagates the
...
if (numPixels < numel(metadata.InstanceData))
    ...
    if ((numPixels+1) < numel(metadata.InstanceData))
        % This accounts for 1 byte of padding that sometimes has to appear.
        warning(message('images:dicomread:tooMuchData'));
    end
    explicitReshape = true;

elseif (numPixels > numel(metadata.InstanceData)) && ...
        (~isempty(metadata.NumberOfFrames)) && ...
        ((numPixels / metadata.NumberOfFrames) == numel(metadata.InstanceData))

    % Some nonconforming DICOM files report more frames than are stored in
    % the file.
    numPixels = numel(metadata.InstanceData);
...
elseif strcmpi(func2str(decodeFcn), 'decompressVideo')
    % 
    X = decodeFcn(metadata, offset, false, frames);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function tf = needToSwapRLE

persistent swapRLE
if ~isempty(swapRLE)
    tf = swapRLE;
else
    % RLE data is stored in Composite Pixel Codes. Here's how this is
    % described in PS 3.5 Appendix G.2: "The first Segment is generated by
    % stripping off the most significant byte of each Padded Composite
    % Pixel Code and ordering these bytes sequentially." Consequently,
    % multi-byte RLE data is stored big-endian.
    swapRLE = isequal(getEndianOfMachine, 'L');
    tf = swapRLE;
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [X, prevWarning] = decompressVideo(metadata, offset, prevWarning, framesToRead)

    % The offset value supplied already points to the start of the Item
    % delimiter which stores the video data.
    ...
    % The delimiter has the sequence [254 255 0 224]. This corresponds to
    % (FFFE,E000) once you flip the bytes to account for endianness.
    expDelimiter = uint8([254 255 0 224]);
    isMoreVideoPresent = isequal(metadata.InstanceData(offset+1:offset+4), expDelimiter);

    while isMoreVideoPresent
        % Skip the item delimiter, moving to the start of the codestream.
        offset = offset + 4;
        
        % The entire set of frames can be stored as separate Items i.e.
        % separated by (FFFE, E000) markers. Each of these Items
        % corresponds to a completely formed video. We need to ensure that
        % all these Items are read.
    
        % Get the length of the encoded data segment.
        len = metadata.InstanceData(offset+1:offset+4);
        if needToSwapOffsetTable
            len = swap32Bit(len);
        end
        len = images.internal.dicom.typecast(len, 'uint32');
    
        % Move to the start of the encoded stream by skipping 4 bytes for
        % the length
        offset = offset + 4;
        
        % Each of these video frame chunks can be appended and stored in
        % the same video file. 
        fwrite(fid, metadata.InstanceData(offset+1:offset+len), 'uint8');
        
        % Update the offset to the end of the encoded stream
        offset = offset + len;

        % Verify if the byte stream now contains the expected delimiter
        % that signals the start of the next data item segment
        isMoreVideoPresent = isequal(metadata.InstanceData(offset+1:offset+4), expDelimiter);
    end
    fclose(fid);

    % Throw a clear exception if VideoReader object creation fails.
    try
        % VideoReader has a bug on Mac OS where it expects a suitable
        % extension for the file depending upon the encoding of the video.
        % For example, an MPEG-4 container must have a *.mp4 extension and
        % MPEG-2 container must have a *.mpg extension. It fails otherwise.
        % See g2588708 for details. As a workaround, try both of these
        % extensions before erroring out.
        if ismac
            try
                fileNameToUse = [tempfile '.mp4'];
                movefile(tempfile, fileNameToUse);
                vidObj = VideoReader(fileNameToUse);
    
                % If VideoReader call succeeds, then install a callback to
                % delete the file upon completion. If it fails, a different
                % extension will be tried and the file will be deleted by that
                % code.
                tmp = onCleanup( @() delete(fileNameToUse) );
            catch ME
                oldFileNameToUse = fileNameToUse;
                fileNameToUse = [tempfile '.mpg'];
                movefile(oldFileNameToUse, fileNameToUse);
    
                % Install a callback to delete the file.
                tmp = onCleanup( @() delete(fileNameToUse) );
            ...
            tmp = onCleanup(@() delete(tempfile));
            vidObj = VideoReader(tempfile);
        end
    catch ME
        error(message("images:dicomread:videoreadfailed"));
    end

    % Read all frames
    if isempty(framesToRead)
        X = read(vidObj);
    else
        % Check if the frame index specified is within the bounds
        if any(framesToRead > vidObj.NumFrames)
            error( message( "images:dicomread:frameIndexOutOfBounds", ...
                            vidObj.NumFrames ) );
        end

        if isscalar(framesToRead)
            X = read(vidObj, framesToRead);
        else
            % VideoReader/read accepts either a range of frames to read or
            % specific frame indices. So if multiple frames are requested,
            % we have to read them one at a time
            X = zeros( vidObj.Height, vidObj.Width, 3, ...
                       numel(framesToRead), "uint8" );
            for cnt = 1:numel(framesToRead)
                X(:, :, :, cnt) = read(vidObj, framesToRead(cnt));
            end
        end
...
for p = 1:segmentOffsets(1)
    ...
    [pixCodes(p,:), prevWarning] = ...
        images.internal.builtins.dicom_decode_rle_segment( metadata.InstanceData(start:stop), ...
...
txfrDetails = images.internal.dicom.dicom_uid_decode(transferSyntaxUID);
...
% If descriptors are incompletely specified, there is no colormap.
if (numel(metadata.RedPaletteLUTDescriptor)~=3) || ...
        (numel(metadata.GreenPaletteLUTDescriptor)~=3) || ...
        (numel(metadata.BluePaletteLUTDescriptor)~=3)
...
if ((isempty(metadata.OverlayBitsAllocated)) || ...
        ...
        (isempty(metadata.OverlayBitPosition)))
...
if isempty(metadata.NumberOfFramesInOverlay)
    ...
    for p = 1:numOverlays
        ...
        if (metadata.OverlayBitPosition{p} == 0)
            ...
            if isempty(metadata.OverlayData)
                % OverlayData field doesn't exist for 0 OverlayBitPosition
                overlays = [];
                return;
            end
            if ((rows ~= imageRows) || (cols ~= imageCols))
                ...
                tmp = images.internal.builtins.tobits(metadata.OverlayData{attrOverlayCount});
...
if (metadata.OverlayBitPosition{1} == 0)
    ...
    tmp = images.internal.builtins.tobits(metadata.OverlayData{1});
