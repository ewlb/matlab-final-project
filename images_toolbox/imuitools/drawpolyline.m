function h = drawpolyline(varargin)
%drawpolyline Create draggable, reshapable polyline ROI
%    H = drawpolyline begins interactive placement of a polyline region of
%    interest (ROI) on the current axes. The function returns H, a handle to
%    an image.roi.Polyline object. You can modify the ROI interactively using
%    the mouse.  The ROI also supports a context menu that controls aspects
%    of its appearance and behavior.
%
%    H = drawpolyline(AX,____) creates the ROI in the axes specified by AX
%    instead of the current axes (gca).
%
%    H = drawpolyline(____, Name, Value) modifies the appearance of the ROI
%    using one or more name-value pairs.
%
%    During interactive placement, left-click to add vertices as you move
%    the mouse over the image. To finish (close) the polyline, double-click.
%
%    To add a vertex after finishing a polyline, position the cursor over the
...
%
%    'Position'        Position of the polyline, specified as an n-by-2
%                      array of the form [x1 y1; ...; xn yn] where each row
%                      specifies the position of a vertex of the polyline.
%
...
%
%    % Begin interactive placement of a polyline
%    h = drawpolyline();
%
%    % Set the width of the polyline to be small
%    h.LineWidth = 1;
%
%
%    See also: images.roi.Polyline, drawcircle, drawellipse, drawfreehand,
%    drawline, drawpoint, drawpolygon, drawrectangle, drawassisted,
%    drawcuboid

% Copyright 2018-2020 The MathWorks, Inc.

% Create ROI using formal interface
h = images.roi.Polyline(varargin{:});
