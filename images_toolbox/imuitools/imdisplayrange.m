...
images.internal.legacyui.utils.reactToImageChangesInFig(imageHandles,hPanel,...
...
if nargout > 0
    ...
    function createStaticDisplayRange
        ...
        % g3133338: For display ranges that are large or floating point,
        % the range panel does not accomodate the text.
        if hRange.Position(3) < hRange.Extent(3)
            adjustDisplayRangePosition();
        end
    ...
    %-------------------------
    function adjustDisplayRangePosition

        delta = hRange.Extent(3) - hRange.Position(3);
        % Expand range text right
        hRange.Position(3) = hRange.Position(3) + delta;
        % Expand range panel left
        hPanel.Position(1) = hPanel.Position(1) - delta;
        hPanel.Position(3) = hPanel.Position(3) + delta;
