...
classdef BrowserThumbnails < images.internal.app.utilities.thumbnail.Thumbnails
    ...
    methods
        function thumbs = BrowserThumbnails(hParent, tSize)
            thumbs@images.internal.app.utilities.thumbnail.Thumbnails(hParent, [1 1]);
    ...
    methods % Abstract and overriding implementations of base class methods
        function positionsInvalidated(thumbs)
            if ~isvalid(thumbs)
                return;
            end
            
            positionsInvalidated@images.internal.app.utilities.thumbnail.Thumbnails(thumbs);
    ...
    methods % Custom color markings
        function enBolden(thumbs, imageNum)
            ...
            topLeftYXs = thumbs.getTopLeftYX(imageNum);
