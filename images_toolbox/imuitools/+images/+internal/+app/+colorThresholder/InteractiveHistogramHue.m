...
classdef InteractiveHistogramHue < handle
    ...
    methods
        ...
        function obj = InteractiveHistogramHue(hParent, data)
            ...
            % Add label.
            images.internal.app.colorThresholder.addLabelToHistogram(obj.hAx, 'H')
...
function hAx = createHistogram(parentForAxes, histStruct)
    ...
    axesColor = images.internal.app.colorThresholder.histogramAxesColor();
...
addlistener(selectorAPI.maxLine.handle,  'Hit', @(~,~)maxLineButtonDown);
addlistener(selectorAPI.minLine.handle,  'Hit', @(~,~)minLineButtonDown);
addlistener(selectorAPI.maxPatch.handle, 'Hit', @(~,~)maxPatchButtonDown);
addlistener(selectorAPI.minPatch.handle, 'Hit', @(~,~)minPatchButtonDown);
addlistener(selectorAPI.bigPatch.handle, 'Hit', @(~,~)patchButtonDown);
