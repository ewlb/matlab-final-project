        ...
        function this = CameraPropertiesPanel(camObj, imPreviewDisplay)
            ...
            for idx =  1:numProperties
                if ~isempty(props.(propNames{idx}))
                    this.DevicePropObjects.(propNames{idx}) = images.internal.app.colorThresholder.ComboBoxControl(propNames{idx}, props.(propNames{idx}){1}, camObj.(propNames{idx}));
                    ...
                    this.DevicePropObjects.(propNames{idx}) = images.internal.app.colorThresholder.TSSliderEditControl(propNames{idx}, range(1), range(2), camObj.(propNames{idx}));
                    addlistener(this.DevicePropObjects.(propNames{idx}), 'PropValueChanged', @(~,~)updateCameraObjectProps(this, propNames{idx}));
                    position = strcat('xy(4,',num2str(idx*2),')'); 
