...
classdef InteractiveHistogram < handle
    ...
    methods
        ...
        function obj = InteractiveHistogram(hParent, data, varargin)
            ...
            histStruct = images.internal.app.colorThresholder.getHistogramDataRaw(data);
            ...
            % Set up callbacks
            histEvent = images.internal.app.colorThresholder.HistogramChanged();
...
axColor = images.internal.app.colorThresholder.histogramAxesColor();
...
if (~isempty(label))
    images.internal.app.colorThresholder.addLabelToHistogram(hAx, label)
end
...
selectorColor = images.internal.app.colorThresholder.selectorColor;
...
addlistener(selectorAPI.maxLine.handle,  'Hit', @(src,~)minMaxLineDown(src));
addlistener(selectorAPI.minLine.handle,  'Hit', @(src,~)minMaxLineDown(src));
addlistener(selectorAPI.maxPatch.handle, 'Hit', @(src,~)minMaxPatchDown(src));
addlistener(selectorAPI.minPatch.handle, 'Hit', @(src,~)minMaxPatchDown(src));
addlistener(selectorAPI.bigPatch.handle, 'Hit', @(src,~)bigPatchDown(src));
