...
classdef InteractiveHistogram < handle
    ...
    events
       SliderMoved % Event triggered everytime a slider is moved
    end
    ...
    methods
        ...
        function obj = InteractiveHistogram(hParent, data, varargin)
            ...
            histStruct = images.internal.app.colorThresholderWeb.getHistogramDataRaw(data);
            ...
            obj.hPanel = uigridlayout(obj.parent, [1 1] ,...
            ...
            % Set up callbacks
            histEvent = images.internal.app.colorThresholderWeb.HistogramChanged();
            ...
            addlistener(histEvent, 'preChange', @obj.notifySliderMoved);
        ...
        %------
        function notifySliderMoved(obj, ~, ~)
           notify(obj, 'SliderMoved'); 
        end
...
axColor = images.internal.app.colorThresholderWeb.histogramAxesColor();
...
if (~isempty(label))
    images.internal.app.colorThresholderWeb.addLabelToHistogram(hAx, label)
end
...
selectorColor = images.internal.app.colorThresholderWeb.selectorColor;
...
addlistener(selectorAPI.maxLine.handle,  'Hit', @(src,~)minMaxLineDown(src, histEvent));
addlistener(selectorAPI.minLine.handle,  'Hit', @(src,~)minMaxLineDown(src, histEvent));
addlistener(selectorAPI.maxPatch.handle, 'Hit', @(src,~)minMaxPatchDown(src, histEvent));
addlistener(selectorAPI.minPatch.handle, 'Hit', @(src,~)minMaxPatchDown(src, histEvent));
addlistener(selectorAPI.bigPatch.handle, 'Hit', @(src,~)bigPatchDown(src, histEvent));
...
%====================================
    function minMaxLineDown(src, histEvent, varargin)
        
        notify(histEvent, 'preChange');
        ...
        function minMaxLineMove(~,varargin)
            ...
            xpos = double(images.internal.app.utilities.getCurrentPoint(hAx));
            if isMaxLine
                newMax = xpos;
...
%======================================
    function minMaxPatchDown(src, histEvent, varargin)
        ...
        startX = images.internal.app.utilities.getCurrentPoint(hAx);
        oldX = srcLine.get();
        ...
        function minMaxPatchMove(~, varargin)
            ...
            newX = double(images.internal.app.utilities.getCurrentPoint(hAx));
            delta = newX - startX;
...
%==============================
    function bigPatchDown(src, histEvent, varargin)
        ...
        function bigPatchMove(varargin)
            ...
            newX = images.internal.app.utilities.getCurrentPoint(hAx);
...
% problem with the painters renderer and patches at the edge (g298973).
