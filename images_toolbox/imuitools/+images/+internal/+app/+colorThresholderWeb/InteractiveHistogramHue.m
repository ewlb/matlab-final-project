...
classdef InteractiveHistogramHue < handle
    ...
    methods
        ...
        function obj = InteractiveHistogramHue(hParent, data)
            ...
            % Add label.
            images.internal.app.colorThresholderWeb.addLabelToHistogram(obj.hAx, 'H');
...
function hAx = createHistogram(parentForAxes, histStruct)
    ...
    axesColor = images.internal.app.colorThresholderWeb.histogramAxesColor();
...
% selector also has minimum and maximum arrow-shaped patches that correspond
...
addlistener(selectorAPI.maxLine.handle,  'Hit', @(~,~)maxLineButtonDown(histEvent));
addlistener(selectorAPI.minLine.handle,  'Hit', @(~,~)minLineButtonDown(histEvent));
addlistener(selectorAPI.maxPatch.handle, 'Hit', @(~,~)maxPatchButtonDown(histEvent));
addlistener(selectorAPI.minPatch.handle, 'Hit', @(~,~)minPatchButtonDown(histEvent));
addlistener(selectorAPI.bigPatch.handle, 'Hit', @(~,~)patchButtonDown(histEvent));
...
initCursorChangeOverDraggableObjs(hFig, draggableObjList, 'hand');
    ...
    %==============================
    function patchButtonDown(histEvent, varargin)
        ...
        [startX, startY] = images.internal.app.utilities.getCurrentPoint(hAx);
        ...
        function patchMove(varargin)
            ...
            [newX, newY] = images.internal.app.utilities.getCurrentPoint(hAx);
    ...
    %==============================
    function minLineButtonDown(histEvent, varargin)
    ...
    %==============================
    function minPatchButtonDown(histEvent, varargin)
    ...
    %==============================
    function maxLineButtonDown(histEvent, varargin)
    ...
    %==============================
    function maxPatchButtonDown(histEvent, varargin)
