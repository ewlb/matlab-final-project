...
%  This function is called by IMCONTRAST, WINDOWLEVEL, and IMHISTPANEL.
...
% Find the min and max and replace +Inf with realmax and -Inf with realmin
% to avoid multiplying 0 and Inf, which is undefined, at line 162 below:
%    bins = round(bins .* origRange + xMin);      (g1485712)
xMin = min(X(:));
if isinf(xMin)
    if xMin < 0
        xMin = realmin(class(xMin));
    else
        xMin = realmax(class(xMin));
    end
end
xMax = max(X(:));
if isinf(xMax)
    if xMax < 0
        xMax = realmin(class(xMax));
    else
        xMax = realmax(class(xMax));
    end
end
origRange = xMax - xMin;
...
[counts,idxOfBinsWithImageData] = saturateOverlyRepresentedCounts(counts,nbins);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [counts,idxOfImage] = saturateOverlyRepresentedCounts(counts,nbins)
