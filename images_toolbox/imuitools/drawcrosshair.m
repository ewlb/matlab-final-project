function h = drawcrosshair(varargin)
%drawcrosshair Create draggable crosshair ROI
%    H = drawcrosshair begins interactive placement of a crosshair region of
%    interest (ROI) on the current axes. The function returns H, a handle to
%    an images.roi.Crosshair object. You can modify an ROI interactively using
%    the mouse. The ROI also supports a context menu that controls
%    aspects of its appearance and behavior.
%
%    H = drawcrosshair(AX,____) creates the ROI on the axes specified by AX
%    instead of the current axes (gca)
%
%    H = drawcrosshair(____, Name, Value) modifies the appearance of the ROI
...
% 
%    % Begin interactive placement of a crosshair
%    h = drawcrosshair();
% 
...
%
%
%    See also: images.roi.Crosshair, drawpoint, drawcircle, drawellipse,
%    drawfreehand, drawline, drawpolygon, drawpolyline, drawrectangle,
%    drawassisted, drawcuboid

% Copyright 2019-2020 The MathWorks, Inc.

% Create ROI using formal interface
h = images.roi.Crosshair(varargin{:});
