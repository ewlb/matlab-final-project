...
%RSet  Reduced resolution pyramidal data set.
...
% Copyright 1993-2020 The MathWorks, Inc.
    ...
    methods (Access = 'public')
        ...
        function levelSizes = getResolutionLevelSizes(obj)
            % Returns a numLevels-by-2 [rows, columns] array describing the
            % size of each resolution level in the file.
            
            maxLevel = obj.getMaxLevel();
            levelSizes = zeros(maxLevel+1, 2);
            
            % Tile and row/column indexing is 0 based.
            L0Failed = false;
            for level = 0:maxLevel
                try
                    units_per_tile = obj.tile_size * (2 ^ level);
                    maxRowTile = getMaxTileNum(obj.base_tiles(1), obj.getFullHeight);
                    maxColTile = getMaxTileNum(obj.base_tiles(2), obj.getFullWidth);
                    
                    data = obj.getTile(level,maxRowTile,maxColTile);
                catch excp
                    % Some very early RSET files did not store the L0 data
                    % in the RSET file. This will likely fail again later
                    % when attempting to work with the instantiated RSet
                    % object, but don't fail in this function since it's
                ...
                % Include last tile size since the last row and column can
                % have elements smaller than tile_size.
                rows = obj.tile_size * maxRowTile  + size(data,1);
                cols = obj.tile_size * maxColTile  + size(data,2);
                levelSizes(level+1,:) = [rows,cols];
            end
            
            if L0Failed && (maxLevel > 0)
                levelSizes(1,:) = [NaN NaN];
            end
