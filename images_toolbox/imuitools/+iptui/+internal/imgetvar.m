...
%  drop-down. CUSTOM can have one of the following values:
%        0    Standard drop-down
%        1    RGB only drop-down
%        2    Intensity only drop-down
%        3    Binary only drop-down
%        4    MxNxP volume only drop-down 
%        5    DICOM Collection only drop-down
%        6    MxNxP labeled volume only drop-down
...
%   Copyright 2004-2018 The MathWorks, Inc.
  ...
  function hPanel = createFilterMenu
    ...
    elseif mode==4
        image_type_str = {getString(message('images:privateUIString:imageTypeVolume'))};
    elseif mode==5
        image_type_str = {getString(message('images:privateUIString:imageTypeDicomCollection'))};
    elseif mode==6
        image_type_str = {getString(message('images:privateUIString:imageTypeLabeledVolume'))};
    end
  ...
  function hPanel = createImportPanel(im_type)
    ...
    elseif mode==4
        varInd = iptui.internal.filterWorkspaceVars(workspace_vars,'volume');
    elseif mode==5
        varInd = iptui.internal.filterWorkspaceVars(workspace_vars,'dicomCollection');
    elseif mode==6
        varInd = iptui.internal.filterWorkspaceVars(workspace_vars,'labeledVolume');
    end
  ...
  function status = getVars
    ...
    if colormap_is_specified
      ...
      if (length(sz) ~= 2 || sz(2) ~= 3)
        error_str = getString(message( ...
            'images:privateUIString:doesNotQualityAsColormapStr',cmap_str));
        errordlg(error_str);
        status = FAILURE;
        return;
      end
