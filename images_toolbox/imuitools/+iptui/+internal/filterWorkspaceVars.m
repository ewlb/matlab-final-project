...
%   'indexed', 'intensity', 'binary', 'all', 'volume', 'dicomCollection'.
%   ('all' does not include 'volume' or 'dicomCollection'.)  OUT is an
%   array of indices into  WS_VARS of variables that match the filter
%   specification.

%   Copyright 2004-2018 The MathWorks, Inc.

valid_filters = {'colormap','rgb','colorThresholderRGB','imageSegmenterIntensity','indexed','intensity','binary','volume','all','dicomCollection','labeledVolume'};
...
switch lower(filter)
    ...
    case 'volume'
        ...
        for n = 1:num_of_vars
            if isNotEmpty(ws_vars(n)) && isVolume(ws_vars(n))
                out = [out,n]; %#ok<AGROW>
            end
        end
    ...
    case 'labeledvolume'
        ...
        for n = 1:num_of_vars
            if isNotEmpty(ws_vars(n)) && isLabeledVolume(ws_vars(n))
                out = [out,n]; %#ok<AGROW>
            end
        end
    ...
    case 'dicomcollection'
        ...
        for n = 1:num_of_vars
            theVar = ws_vars(n);
            if isNotEmpty(theVar) && isDicomCollection(theVar)
                out = [out,n]; %#ok<AGROW>
            end
        end
    ...
    function TF = isValidColorThresholderRGB(var_struct)
        ...
        supportedClasses = {'double', 'single', 'uint8', 'uint16'};
    ...
    function TF = isVolume(var_struct)
        
        is_M_by_N_by_P = length(var_struct.size) == 3 && all(var_struct.size > 1);
        
        is_valid_type = any(strcmpi(var_struct.class, [default_classes, {'logical','int16','uint32','int32'}]), 2);
        
        TF = is_M_by_N_by_P && is_valid_type;
        
    end

    function TF = isLabeledVolume(var_struct)
        
        is_M_by_N_by_P = length(var_struct.size) == 3 && all(var_struct.size > 1);
        
        supportedClasses = [default_classes, {'categorical','int16','uint32','int32'}];
        is_valid_type = any(strcmpi(var_struct.class, supportedClasses), 2);
    ...
    function tf = isDicomCollection(varStruct)
        switch (varStruct.class)
        case 'table'
            tf = evalin('base', sprintf('images.internal.app.dicom.isDicomCollection(%s);', varStruct.name));
        otherwise
            tf = false;
        end
    end
