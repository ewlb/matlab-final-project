...
%    H = drawline begins interactive placement of a line region of interest
%    (ROI) on the current axes. The function returns H, a handle to an
%    images.roi.Line object. You can modify the ROI interactively using the
%    mouse. The ROI also supports a context menu that controls aspects of
%    its appearance and behavior.
%
%    H = drawline(AX,____) creates the ROI in the axes specified by AX
%    instead of the current axes (gca).
%
%    H = drawline(____, Name, Value) modifies the appearance of the ROI
...
%
%    'Position'        Position of the line, specified as a 2-by-2 array
%                      of the form [x1 y1; x2 y2] where each row specifies
%                      the respective end-point of the line segment that
%                      defines the line ROI. This property updates
...
%
%    % Begin interactive placement of a line that turns yellow when selected
%    h = drawline('SelectedColor','yellow');
%
%    % Set Selected to be true (this can also be done by clicking on the ROI)
%    h.Selected = true;
%
%
%    Example 2
...
%    h = drawline('Position',[1000,1000;1000,1200],'StripeColor','white');
%
%
%    See also: images.roi.Line, drawcircle, drawellipse, drawfreehand,
