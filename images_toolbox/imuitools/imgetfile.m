...
try
    ...
    if strcmp(exception.identifier,'MATLAB:cd:DirectoryNameTooLong')
        % On Windows, which has a relatively short maximum path length,
        % either cached_path or initialPath could be too long to use
        % directly, even though the uigetfile dialog can allow navigating
        % to the destination by clicking into successive folders. See
        % MathWorks record g3234938. Try the uigetfile again without
