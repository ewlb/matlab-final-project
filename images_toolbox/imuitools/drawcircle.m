function h = drawcircle(varargin)
%drawcircle Create draggable, resizable circular ROI
%    H = drawcircle begins interactive placement of a circular region of
%    interest (ROI) on the current axes. The function returns H, a handle to
%    an images.roi.Circle object. You can modify the ROI interactively using
%    the mouse. The ROI object supports a context menu that controls
%    aspects of its appearance and behavior.
%
%    H = drawcircle(AX,____) creates the ROI on the axes specified by AX
%    instead of the current axes (gca).
%
%    H = drawcircle(____, Name, Value) modifies the appearance of the ROI
%    using one or more name-value pairs.
%
%    Parameters include:
%
%    'Center'          Center of the ROI, specified as a 1-by-2 array of the
%                      form [x y]. The values x and y are the coordinates of
%                      the ROI center point. The value of this property
%                      changes automatically when you draw or move the ROI.
%
...
%                      specified as true (default) or false. When true, you
%                      can delete the ROI via the context menu. To disable
%                      this context menu item, set Deletable to false. Even
%                      when set to false, you can still delete the ROI by
%                      calling the delete function specifying the handle to
%                      the ROI, delete(H).
%
...
%                      as true or false. When true (default), you
%                      can select the ROI face. When false, you cannot
%                      select the ROI face by clicking.
%
...
%                      When this property is empty (the default), the ROI
%                      has no label.
%
...
%
%    'Radius'          Radius of the circle, specified as numeric scalar.
%                      The value of this property changes automatically
%                      when you draw or move the ROI.
%
%    'Selected'        Selection state of the ROI, specified as a logical
%                      scalar value. To set this property to true
%                      interactively, click the ROI. To clear the selection
%                      of the ROI, and set this property to false,
%                      ctrl-click the ROI.
%
...
%                      'none', the Color property specifies ROI color,
...
%
%    % Begin interactive placement of a black circle
%    h = drawcircle('Color','k','Label','My Circle');
%
%    % Prevent users from deleting the ROI using the context menu
%    h.Deletable = false;
%
%
%    Example 2
...
%
%
%    See also: images.roi.Circle, drawellipse, drawfreehand, drawline,
%    drawpoint, drawpolygon, drawpolyline, drawrectangle, drawassisted,
%    drawcuboid
...
if isempty(h.Center) || isempty(h.Radius)
    if ~isempty(h.Center)
        warning(message('images:imroi:incompleteDefinition','Center'));
    end
    if ~isempty(h.Radius)
        warning(message('images:imroi:incompleteDefinition','Radius'));
    end
