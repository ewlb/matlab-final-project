function hfigure = imcontrast(handle, varargin)
...
s = settings;
if strcmpi(class(getCanvas(ancestor(handle,'figure','toplevel'))),'matlab.graphics.primitive.canvas.HTMLCanvas') && ...
        s.images.ImcontrastUseWebApp.ActiveValue
    hfigure = images.internal.imcontrast.imcontrastWebImpl(handle,rsetMode);
else
...
images.internal.legacyui.utils.reactToImageChangesInFig(imageHandle,hFig,...
...
%==============================
    ...
    function closeHistFig(~,~)
        ...
        % This is necessary to resolve a graphics bug where the default
        % interactivity state does not get reset if the WindowButtonUpFcn
        % is cleared before the image ButtonDownFcn
        drawnow;
