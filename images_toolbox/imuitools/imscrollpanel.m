...
%                              image replacement.
%
%       Additional parameters include 'Colormap', 'DisplayRange', and
...
% newLocationCallbackFunctions is used by sendNewLocation() to
...
setappdata(hScrollpanel,'API',api);
    ...
    function updateAxesPosition(xmin,ymin)
        ...
        if isFullImageWShowing()
            ...
            % the complete image is centered in the view, there is a left
...
iptcheckhandle(parent,...
    ...
    mfilename,'HPARENT',1)
checkAutoResizeChildren(parent);
...
%----------------------------------
function checkAutoResizeChildren(p)

auto_resize_enabled = false;
while (~isempty(p))
    if isprop(p,"AutoResizeChildren")
        auto_resize_enabled = p.AutoResizeChildren;
...
if auto_resize_enabled
    error(message("images:imscrollpanel:autoResizeChildrenEnabled"))
end

end % checkAutoResizeChildren
