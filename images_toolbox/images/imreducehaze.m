...
%   D = IMREDUCEHAZE(I) reduces atmospheric haze in RGB or grayscale image 
%   I. D is the dehazed image. 
%
%   [D, T, L] = IMREDUCEHAZE(I) reduces atmospheric haze in RGB or
%   grayscale image I. D is the dehazed image. T contains an estimate
...
%
%   [___] = IMREDUCEHAZE(I, AMOUNT) alters the AMOUNT of haze to be reduced.
%   AMOUNT is a scalar value in the range [0, 1]. When AMOUNT is 1, the 
%   maximum amount of haze is reduced. When AMOUNT is 0, the input image is 
%   unchanged. The default value is 1.
%
...
%
%     'AtmosphericLight'     A scalar (for grayscale) or a 3-element vector
%                            (for RGB images) in the range [0,1], specifying
%                            the maximum value to be treated as haze. When 
%                            not specified, this value is computed based on 
%                            the technique specified by 'Method'. 
%
%     'ContrastEnhancement'  A post-processing technique to improve image
%                            contrast: 'global' (default), 'boost', or 'none'.
%                            'global' performs a global linear contrast
%                            stretching clipping saturated pixel values in 
%                            both the highlights and the shadows. 'boost'
%                            improves the image contrast according to the 
%                            specified 'BoostAmount'. 'none' performs no
%                            contrast enhancement on the dehazed image.
%
...
%   Notes
%   -----
%   When the value of 'AtmosphericLight' is not specified, the default
%   value is estimated based on the method for reducing haze. For
%   approxdcp, the brightest 0.1% pixels of the dark channel are considered
%   to estimate the value. For simpledcp, the default value is computed
%   using quadtree decomposition.
%   
%   Class Support
%   -------------
%   The input array I must be one of the following classes: uint8, uint16, 
%   single or double. It must be nonsparse and non empty. Output image D is
%   an array of the same size and type as I. T is a double 2D array of the 
%   same height and width as I. L is a double array of the size 1-by-3. 
%   
%   References
%   ---------
...
%   % Example 2
%   % ---------
%   % Reduce haze 90% using approxdcp and global contrast stretching.
...
%   % Example 3
%   % ---------
%   % Estimate the haze thickness and image depth.
...
% 
% See also imread, imshowpair, imhistmatch, imguidedfilter
...
if isempty(x) && isempty(y)
    [~, idx] = max(darkChannel(:));
    [row, col] = ind2sub(size(darkChannel),idx);
    atmosphericLight = A(row, col, :);
else
    [~, maxIdx] = max(grayVals);
