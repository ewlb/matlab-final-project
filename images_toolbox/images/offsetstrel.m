    ...
    methods
        ...
        function decomp = decompose(obj)
            ...
            if ~isempty(obj)
                ...
                counter = 1;
                for j = 1:length(obj)
                    strelseq = obj(j).OffsetSEHolder.decompose();
                    decomp = [decomp repmat(offsetstrel, [1 numel(strelseq)])];
                    for k = 1:numel(strelseq)
                        decomp(counter).OffsetSEHolder = strelseq(k);
                        counter = counter+1;
                    end
                end
            else
        ...
        function ose = reflect(obj)
            ...
            if ~isempty(obj)
                obj = obj(:);
                for id = 1:numel(obj)
                    ose(id).OffsetSEHolder = obj(id).OffsetSEHolder.reflect();
                    ose(id).Type = obj(id).Type;
                end
            else
                ose(1) = [];
            end
        ...
        function ose = translate(obj, displacement)
            ...
            if ~isempty(obj)
                obj = obj(:);
                for id = 1:numel(obj)
                    ose(id).OffsetSEHolder = obj(id).OffsetSEHolder.translate(displacement);
                    ose(id).Type = obj(id).Type;
                end
            else
                ose(1) = [];
