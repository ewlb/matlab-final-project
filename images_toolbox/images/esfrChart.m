...
%
%   An esfrChart encapsulates the Extended/Enhanced/WedgeExtended/WedgeEnhanced
%   versions of the edge spatial frequency response test chart specified in 
%   ISO 12233:2014 standard.
%
% esfrChart Create an esfrChart object from an image.
%
%   CHART = esfrChart(A) creates an esfrChart object CHART
...
%                             esfrChart performs automatic chart
...
%   -----
%
%   1.  The esfrChart is an extended, enhanced, wedge-extended or  
...
classdef esfrChart
    ...
    methods
        function chart = esfrChart(varargin)
            %   CHART = esfrChart(A) creats an esfrChart object CHART
            %   from image A. A can be an RGB or grayscale image.
            %
            %   CHART = esfrChart(___, Name, Value, ___) constructs an
            %   esfrChart object CHART where additional parameters
        ...
        function [colorTable, varargout] = measureColor(chart)
            ...
            %   -----
            %
            %   1.  An esfrChart should be correctly imported for accurate and
            ...
            if (isequal(chart.Image(:,:,1),chart.Image(:,:,2),chart.Image(:,:,3)))
                warning(message('images:esfrChart:grayChartColorMetrics'));
            end
...
if (~isempty(options.RegistrationPoints) & ~isempty(options.Sensitivity))
    error(message('images:esfrChart:invalidArgumentPair'));
end
...
if (checkCVT == 0)
    error(message('images:esfrChart:noInstallCVT'));
end
attributes = {'scalar'};
...
function [RegistrationPoints, modelPoints, Style]=estimateDistortedChart(chartUndistorted,CameraParameters,RegistrationPoints,Style)
    %undistorted image 
    [chartUndistorted.Image, newIntrinsics] = undistortImage(chartUndistorted.Image,CameraParameters,'OutputView','full');
    newOrigin = CameraParameters.Intrinsics.PrincipalPoint - newIntrinsics.PrincipalPoint;
