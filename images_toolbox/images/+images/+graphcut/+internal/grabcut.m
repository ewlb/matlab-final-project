        ...
        function [foregroundWeights, backgroundWeights] = computeTerminalEdgeWeights(self,varargin)
            ...
            if ~isempty(trimapUnknownPixels) && isBoundingBoxValid(self)
                ...
                for idx = 1:self.NumGaussianMixtures
                    D = D + images.internal.builtins.grabcut(trimapUnknownPixels, fGMM(idx).Pi,...
                ...
                for idx = 1:self.NumGaussianMixtures
                    D = D + images.internal.builtins.grabcut(trimapUnknownPixels, bGMM(idx).Pi,...
    ...
    methods (Access = 'private')
        ...
        function self = initializeGrabCut(self)
            ...
            [foregroundClusterIndex, self.ForegroundClusterCenters] = images.internal.ocvkmeans(...
                single(self.MeanFeatures(self.SuperpixelsInsideBbox,:)),self.NumGaussianMixtures,self.NumClusterReplicates, 100, 0.0001);
            
            [~,self.BackgroundClusterCenters] = images.internal.ocvkmeans(...
                single(self.MeanFeatures(self.SuperpixelsOutsideBbox,:)),self.NumGaussianMixtures,self.NumClusterReplicates, 100, 0.0001);
        ...
        function [inBbox, outBbox] = findSuperpixelsInsideBoundingBox(self,bbox)
            ...
            [inBbox,outBbox] = images.internal.builtins.uniqueLabelsInMask(self.LabelMatrix,in,self.NumNodes);
            inBbox(inBbox == 0) = [];
            outBbox(outBbox == 0) = [];
