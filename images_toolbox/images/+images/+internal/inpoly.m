...
% (e.g. loops, multiply connected polygons), use inpolygon.
%
...
% Cast all the points to the datatype of xp
if(class(xp)=="double")
    yp = double(yp);
    x = double(x);
    y = double(y);
elseif (class(xp)=="single")
    yp = single(yp);
    x = single(x);
    y = single(y);
else
    assert(false,"Invalid class of polygon points");
end
...
if numel(xp) == 2
    ...
    out = images.internal.builtins.inpoly(x(:),y(:),xp(:),yp(:));
end
