...
classdef TiffAdapter < ImageAdapter
    ...
    methods
        ...
        function obj = TiffAdapter(filename, mode, varargin)
            ...
            validatestring(mode,{'r','r+','w','w8'},'TiffAdapter','Mode',2);
            
            obj.Filename = filename;
            obj.Mode = lower(mode);
            obj.TileCache = images.internal.TileCache();
            
            if strcmpi(mode, 'w') || strcmpi(mode, 'w8')
        ...
        function [] = create(obj)
            ...
            if length(obj.ImageSize) == 3 && obj.ImageSize(3) == 3
                ...
                if isa(obj.FillVal, 'int8') || ...
                    ...
                    % Interpolation. In this case, SamplesPerPixel is 3, 
                    % but MinIsBlack corresponds to only one sample. 
                    % Create a [1x2] array of EmptySamples to represent the
                    % two remaining samples.
                    obj.Photom = Tiff.Photometric.MinIsBlack;
                    tagStruct.ExtraSamples = [Tiff.ExtraSamples.Unspecified, Tiff.ExtraSamples.Unspecified];
                end
