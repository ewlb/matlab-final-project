...
classdef Jp2Adapter < ImageAdapter
    ...
    methods
        ...
        function obj = Jp2Adapter(filename, mode, varargin)
            ...
            obj.id = images.internal.builtins.jp2dispatcher('create', ...
        ...
        %--------------------------------------------------
        function close(obj)
            images.internal.builtins.jp2dispatcher('destroy', obj.id);
        end % close

        %--------------------------------------------------
        function result = readRegion(obj, start, count)
            result = images.internal.builtins.jp2dispatcher('readRegion', ...
                        obj.id, start, count);
        end % readRegion

        %--------------------------------------------------
        function [] = writeRegion(obj, start, data)
            images.internal.builtins.jp2dispatcher('writeRegion', ...
                obj.id, start, data);
        end % writeRegion

        %--------------------------------------------------
        function result = getSize(obj)
            result = images.internal.builtins.jp2dispatcher('getSize', obj.id);
        end % getSize
