...
function [tform,peak] = resolveSimilarityRotationAmbiguityNGC(movingIn,fixedIn,S,thetad)%#codegen
    ...
    if flip_inputs
        S = 1/S;
        thetad = -thetad;
        [moving,fixed] = swap(movingIn,fixedIn);
    else
        moving = movingIn;
        fixed = fixedIn;
    end
    ...
    [tform_translation_1,peak1] = images.internal.coder.findTranslationNGC(scaledRotatedMoving1,fixed);
    [tform_translation_2,peak2] = images.internal.coder.findTranslationNGC(scaledRotatedMoving2,fixed);
