classdef NeighborhoodProcessor < handle %#codegen
    ...
    methods
        function nhoodObj = NeighborhoodProcessor(imSize, nhConnectivity, varargin)
            ...
            % Since, Neighborhood of structuring element as input for morphop is always fixed to 3d unbouunded,
            % nhConn variable needs to be declared as variable size 3d when generating portableC code for morphop
            nhoodObj.isNhoodVarsize3d = coder.const(~coder.internal.isConst(nhConnectivity) && ...
                ~coder.internal.isConst(size(nhConnectivity)) && numel(size(nhConnectivity)) == 3 && ...
                ~coder.internal.isConst(size(nhConnectivity, 3)));
            
            if nhoodObj.isNhoodVarsize3d
                % Since, propagated strel neighborhood fixed to be 3d unbounded,
                % It needs to fix nhConn as variable size 3d unbounded.
                coder.varsize('nhConn', [inf inf inf]);
            end
            ...
            if(ndims(nhConnectivity)>numel(imSize))
                ...
                nhConn   = false(connSize(1:numel(imSize)));
            ...
            nhoodObj.ImageNeighborLinearOffsets = zeros(numberOfNeighbors ,1,coder.internal.indexIntClass());
            nhoodObj.NeighborLinearIndices      = zeros(numberOfNeighbors ,1,coder.internal.indexIntClass());
            if ~nhoodObj.isNhoodVarsize3d
                nhoodObj.NeighborSubscriptOffsets   = zeros(numberOfNeighbors , numel(size(nhConn)), coder.internal.indexIntClass());
            else
                nhoodObj.NeighborSubscriptOffsets   = zeros(numberOfNeighbors , ndims(nhConn), coder.internal.indexIntClass());
            end
        end
    end
                    ...
                    for ind = 1:size(imnhSubs,1)
                        ...
                        for dimInd = 1:ndims(nhoodObj.Neighborhood)
                            ...
                            if(pixelSub(dimInd)<1 || pixelSub(dimInd)>nhoodObj.ImageSize(dimInd))
                                % neighbor is not inside image bounds
                                isInside(ind) = false;
                                % Set out of bound locations to 1
    ...
    methods (Static = true, Access = private)
        ...
        function [loffsets, linds, soffsets, interiorStart, interiorEnd]...
            ...
            % FIXME: change to inline always when g1680778 is fixed.
            coder.inline('never');
            coder.internal.prefer_const(imSize);
            ...
            pixelsPerNhPage     = [1 cumprod(connSize(1:ndims(nhConn)-1))];
            
            % Handle ndims(im)>ndims(nhConn);
            pixelsPerImPage1     = [1 cumprod(imSize(1:end-1))];
            pixelsPerImPage     = pixelsPerImPage1(1:ndims(nhConn));
            
            
            % Subscripts to center pixel
            switch rounding
                case images.internal.coder.NeighborhoodProcessor.NEIGHBORHOODCENTER.TOPLEFT
                    centerPixelSub = floor((connSize(1:ndims(nhConn))+1)./2);
                otherwise
                    % DEFAULT or BOTTOMRIGHT
                    centerPixelSub = ceil((connSize(1:ndims(nhConn))+1)./2);
            end
            centerPixelSub_int32 = coder.internal.indexInt(centerPixelSub);
            
            % Interior pixels - pixel extents which will have full
            % neighborhoods. Handle ndims(im)>ndims(nhConn);
            interiorStart = ones(1, numel(imSize), coder.internal.indexIntClass());
            interiorStart(1:ndims(nhConn)) = centerPixelSub_int32 ...
            ...
            interiorEnd(1:ndims(nhConn)) =...
                ...
                coder.internal.indexMinus(connSize(1:ndims(nhConn)), centerPixelSub_int32)...
                );
            ...
            for pind = 1:numel(nhConn)
                ...
                if(nhConn(pind))
                    ...
                    % If its a valid neighbor, compute its subscript
                    subs = ...
                        images.internal.coder.NeighborhoodProcessor.getSubscriptArrayFromIndUsingNdims(...
                        ndims(nhConn), connSize, pind);
            ...
            weights  = [centerPixelSub_int32(1) centerPixelSub_int32(2:end)-1];
            ...
            soffsets = bsxfun(@minus, soffsets, centerPixelSub_int32);
        ...
        function sub = getSubscriptArrayFromIndUsingNdims(numDims, siz, ndx)
            % ind2sub implementation
            % Modified to return subs as a single row vector
