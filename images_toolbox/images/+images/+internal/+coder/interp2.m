...
%	1. This file is a copy of interp2 from base MATLAB, to support single 
%	   dimensional input image, supporting geck, g1977680.
%   2. Xq and Yq must be the same size. Use MESHGRID to evaluate on on
%      a grid.
%   3. For best performance supply X and Y as vectors.
%   4. The 'cubic' method requires a uniformly-spaced grid, otherwise an
%      error is issued. Use the 'spline' method explicitly for an
%      unevenly-spaced grid.
%   5. The 'spline' method is optimized for Xq and Yq being outputs of
...
if nargin == 7
    % interp2(X,Y,V,Xq,Yq,method,extrap)
    X = varargin{1};
    Y = varargin{2};
    V = varargin{3};
    Xq = varargin{4};
    Yq = varargin{5};
    METHOD = StringToMethodID(varargin{6});
    EXTRAP = false;
    interp2_validate(V,Xq,Yq,METHOD,varargin{7},X,Y);
    extrapval = cast(varargin{7},'like',V);
...
elseif nargin == 6
    ...
    Vq = interp2_dispatch(V,Xq,Yq,METHOD,EXTRAP,extrapval, ...
        unmeshgrid(X,2),unmeshgrid(Y,1));
elseif nargin == 5
    if coder.internal.isCharOrScalarString(varargin{4})
        % interp2(V,Xq,Yq,method,extrapval)
        V = varargin{1};
        Xq = varargin{2};
        Yq = varargin{3};
        METHOD = StringToMethodID(varargin{4});
        EXTRAP = false;
        interp2_validate(V,Xq,Yq,METHOD,varargin{5});
        extrapval = cast(varargin{5},'like',V);
        Vq = interp2_dispatch(V,Xq,Yq,METHOD,EXTRAP,extrapval);
    else
        % interp2(X,Y,V,Xq,Yq)
        X = varargin{1};
...
elseif nargin == 4
    if coder.internal.isCharOrScalarString(varargin{3})
        % interp2(V,N,method,extrap)
        V = varargin{1};
        N = varargin{2};
        METHOD = StringToMethodID(varargin{3});
        Vq = interp2_ntimes(V,N,METHOD);
    else
        % interp2(V,Xq,Yq,method)
        V = varargin{1};
        Xq = varargin{2};
        Yq = varargin{3};
        METHOD = StringToMethodID(varargin{4});
        EXTRAP = default_extrap(METHOD);
        extrapval = cast(coder.internal.nan,'like',V);
        interp2_validate(V,Xq,Yq,METHOD,extrapval);
        Vq = interp2_dispatch(V,Xq,Yq,METHOD,EXTRAP,extrapval);
    end
elseif nargin == 3
    if coder.internal.isCharOrScalarString(varargin{2})
        % interp2(V,method,extrap)
        V = varargin{1};
        N = coder.internal.indexInt(1);
        METHOD = StringToMethodID(varargin{2});
        Vq = interp2_ntimes(V,N,METHOD);
    elseif coder.internal.isCharOrScalarString(varargin{3})
        % interp2(V,N,method)
        V = varargin{1};
        N = varargin{2};
        METHOD = StringToMethodID(varargin{3});
...
eml_invariant(size(V,2) >= 0 && size(V,1) >=0, ...
...
if AUTOGRID
    ...
    if nx < 3 || ny < 3
        genCubicToLinearWarning;
        Vq = interp2_local(LINEAR,V,Xq,Yq,EXTRAP,extrapval);
        return
    end
    ixmax = nx - 1;
    iymax = ny - 1;
    ...
    if nx < 3 || ny < 3
        genCubicToLinearWarning;
        Vq = interp2_local(LINEAR,V,Xq,Yq,EXTRAP,extrapval,X,Y);
        return
    end
...
n = coder.const(eml_max(1,strlength(method)));
...
function genCubicToLinearWarning
if ~coder.setwarningflag && ~coder.internal.isInParallelRegion
    coder.internal.warning( ...
        'MATLAB:mathcgeo_catalog:CubicNeedsThreeWarnId');
end

%--------------------------------------------------------------------------

function v = unmeshgrid(x,k)
coder.inline('always');
coder.internal.prefer_const(x,k);
if ~(eml_is_const(isvector(x)) && isvector(x))
    n = coder.internal.indexInt(size(x,k));
    stride = coder.internal.prodsize(x,'below',k);
    v = coder.nullcopy(zeros(1,n,'like',x));
    for k = 1:n
        v(k) = x(stride*(k - 1) + 1);
    end
else
...
eml_invariant(isincreasing(v), ...
    'Coder:toolbox:NonIncreasing');
%--------------------------------------------------------------------------
function p = isincreasing(x)
n = numel(x);
if n >= 2
    for k = 1:n-1
        if ~(x(k) < x(k+1))
...
p = true;
%--------------------------------------------------------------------------
function dx = EqualSpacing(x)
nx = numel(x);
dx = zeros('like',x);
minx = x(1);
maxx = x(end);
if ~isinf(minx) && ~isinf(maxx)
    % eqsp = (norm(diff(xCol,2),Inf) <= eps(norm(xCol,Inf)));
    d2max = zeros('like',x);
    for k = 1:nx-2
        d2 = abs((x(k + 2) - x(k + 1)) - (x(k + 1) - x(k)));
        if d2 > d2max
            d2max = d2;
        end
    end
    normx = eml_max(abs(minx),abs(maxx));
    if d2max <= 2*eps(normx)
        dx = (maxx - minx)/(nx - 1);
    end
end
