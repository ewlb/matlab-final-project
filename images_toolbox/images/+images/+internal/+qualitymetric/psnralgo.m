function [peaksnr, snr] = psnralgo(A, ref, peakval,hasBatchDim,logFcn)
...
sizeA = size(A);
err = sum((A-ref).^2,dimsToReduce) ./ prod(sizeA(dimsToReduce));
...
if nargout > 1
    ...
    snr = 10*logFcn(mean(ref.^2,dimsToReduce)./err);
end
