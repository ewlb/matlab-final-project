...
matlab.images.internal.errorIfgpuArray( A, T, R, tdims_A, tdims_B, tsize_B, tmap_B, F);
...
if nBlocks == 1
    ...
    clear('G'); % Free memory used by G (which is no longer needed).
...
N = length(varargin);
if N == 0
    error(message('images:tformarray:combinegridCalledNoArgs'));
end
...
if any(D < 2^Nm)
    error(message('images:tformarray:blockDimsTooSmall'));
end
...
if any(blocksubs ~= blockingfactors)
    error(message('images:tformarray:failedToIterate'));
end
...
elseif N > 1 && L > 1
    ...
    error(message('images:tformarray:NLMustBePositive'));
end
...
if ~IsFinitePositiveIntegerRowVector( tdims_A )
    error(message('images:tformarray:invalidTDIMS_A'));
end

if ~IsFinitePositiveIntegerRowVector( tdims_B )
    error(message('images:tformarray:invalidTDIMS_B'));
end
...
if length(unique(tdims_A)) ~= P
    error(message('images:tformarray:nonUniqueTDIMS_A'));
end

if length(unique(tdims_B)) ~= N
    error(message('images:tformarray:nonUniqueTDIMS_B'));
end
...
if isempty(tmap_B)
    ...
    if ~isempty(tsize_B)
        warning(message('images:tformarray:ignoringTSIZE_B'))
              
    end
    
    if ~isa(tmap_B,'double') || ~isreal(tmap_B) || issparse(tmap_B) || ...
          any(isinf(tmap_B(:)))
        error(message('images:tformarray:invalidTMAP_B'))
    end
    
    L = size(tmap_B,N+1);
    
    if isempty(T) && L ~= P
        error(message('images:tformarray:invalidSizeTMAP_B'))
    end
    
    if ndims(tmap_B) ~= PredictNDimsG(N,L)
        error(message('images:tformarray:invalidDimsTMAP_B'))
    end
...
if ~isempty(T)
    if ~istform(T)
        error(message('images:tformarray:invalidTFORMStruct'));
    end
    
    if length(T) > 1
        error(message('images:tformarray:multipleTFORMStructs'));
    end
    
    if T.ndims_in ~= P
        error(message('images:tformarray:dimsMismatchA'));
    end
    
    if T.ndims_out ~= L
        error(message('images:tformarray:dimsMismatchB'));
    end
end

if ~IsFinitePositiveIntegerRowVector( tsize_B )
    error(message('images:tformarray:invalidTSIZE_B'));
end

% tsize_B and tdims_B must have the same length.
if length(tsize_B) ~= N
    error(message('images:tformarray:lengthMismatchB'));
end
...
if (~isnumeric(A) && ~islogical(A)) || issparse(A)
    error(message('images:tformarray:invalidA'));
end
...
if ~isresampler(R) || (length(R) ~= 1)
    error(message('images:tformarray:invalidResampler'));
end

if R.ndims ~= Inf && R.ndims ~= length(tdims_A)
    error(message('images:tformarray:resamplerDimsMismatch'));
end
...
if isempty(F)
    ...
    if ~isa(F,'double') || issparse(F)
        error(message('images:tformarray:invalidF'));
    end
    ...
    if ~q
        error(message('images:tformarray:sizeMismatchFA'));
    end
end
