...
if ~isempty(I)
    ...
    if ismatrix(I)
        ...
        if coder.target('MATLAB')
            intVolume(:,:,2) = images.internal.builtins.integralimage(I);
        else
            intVolume(:,:,2) = integralImage(I);
        end
        intVolume(:,:,2) = images.internal.builtins.integralimage(I);
    else
        outputSize = size(I) + 1;
        intVolume = zeros(outputSize);
        if coder.target('MATLAB')
            intVolume(:, :, 2:end) = cumsum( images.internal.builtins.integralimage(I),3);
        else
            intVolume(:, :, 2:end) = cumsum(integralImage(I),3);
        end
