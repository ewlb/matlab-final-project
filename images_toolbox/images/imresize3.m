...
%
%       IMRESIZE3(C, SCALE, 'nearest') 
%       IMRESIZE3(C, [NUMROWS NUMCOLS NUMPLANES], 'nearest')
%  
%   B = IMRESIZE3(___, Name, Value) supplies additional parameters to the
%   function.
%
%   Parameters include:
%  
...
%   Class Support
%   -------------
%   The input image V must be numeric or categorical and nonsparse.
...
useCategoricalLUTCache = false;
makeOutputLogical = false;
categoricalLUT =[];

% unwrap categorical input
if iscategorical(params.A)
    
   [params.A, categoriesA] = images.internal.categorical2numeric(params.A);
   
   useCategoricalLUTCache = isa(params.A, 'uint8');
   
   if isempty(categoricalLUT) && useCategoricalLUTCache
       categoricalLUT = images.internal.cacheCategoricalLUT(categoriesA);
   end
...
if makeOutputCategorical
     B = images.internal.numeric2categorical(B, categoriesA, categoricalLUT);
end
