...
%
%   BWFILL is not recommended. Use IMFILL instead.
%
...
%
%   See also BWSELECT, IMFILL, REGIONFILL.
...
try
    ...
    [xdata,ydata,I,xi,yi,r,c,style,newFig,fillHoles] = ParseInputs(args{:});
catch ME
    rethrow(ME);
end
...
J = images.internal.builtins.bwfillc(Ip, idxList, style);
...
otherwise
    ...
    if (nargout >= 7)
        warning(message('images:bwfill:tooManyOutputs'));
    end
end
...
case 0
    ...
    if (flag == 0)
        error(message('images:bwfill:noImageFound'))
    end
    newFig = 1;
    [xi,yi] = getpts;
...
case 1
    ...
    [xi,yi] = getpts;
    if (isempty(xi))
        error(message('images:bwfill:interactiveSelectionAborted'))
    end
...
case 2
    if (ischar(varargin{2}))
        if (varargin{2}(1) == 'h')
            ...
            error(message('images:bwfill:invalidInput'))
        end
        ...
        style = varargin{2};
        if ((style ~= 4) && (style ~= 8))
            error(message('images:bwfill:invalidN'))
        end
...
otherwise
    error(message('images:bwfill:tooManyInputs'))
end
...
badPix = find((r < 1) | (r > size(I,1)) | ...
        (c < 1) | (c > size(I,2)));
if (~isempty(badPix))
    warning(message('images:bwfill:outOfRangeCoordinates'));
    r(badPix) = [];
    c(badPix) = [];
end
