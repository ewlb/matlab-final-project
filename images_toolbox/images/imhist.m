...
if ~isreal(a)
    % Only process the real part
    a = real(a);
    warning(message('images:imhistc:ignoreImaginaryPart'))
end
...
if islogical(a)
    ...
    y = y';
elseif isa(a,'int8')
    y = images.internal.builtins.imhistc(images.internal.builtins.int8touint8(a), n, isScaled, top); % Call MEX file to do work.
elseif isa(a, 'int16')
    y = images.internal.builtins.imhistc(images.internal.builtins.int16touint16(a), n, isScaled, top); % Call MEX file to do work.
elseif isa(a, 'int32')
    y = images.internal.builtins.imhistc(images.internal.builtins.int32touint32(a), n, isScaled, top); % Call MEX file to do work.
else
...
% In GUIDE, default axes units are characters. In order for axes
% repositioning to behave properly, units need to be normalized.
...
if (nargin ==2)
    ...
    elseif (size(varargin{2},2) == 3)
      ...
      % Validate length of map
      if (max(a(:)) > size(map,1))
          error(message('images:imhist:invalidMapLength'));
      end
