...
%
%     'FillValues'  -  Scalar value used to fill voxels in the output volume
%                      B that are outside the limits of the rotated volume.
%
...
%   Class Support
%   -------------
%   The input image V can be numeric, logical or categorical. The output
%   image B is of the same class as the input image. ANGLE and W can be
%   numeric.
%
%   Example
%   -------
...
bbox   = 'loose';
fillval = 0;
% Support for categorical falls through as imwarp supports categorical. No
% additional changes are required in imrotate3
if iscategorical(V)
    fillval = missing;
    method = 'nearest';
end
...
arg = validatestring(arg,strs);
idx = find(strcmp(arg,strs));
