...
%                               this value smooths the image more.
%
...
%   Class Support
%   -------------
%   The input array A must be one of the following classes: uint8, int8,
%   uint16, int16, uint32, int32, single or double. It must be real and
%   non-sparse. Double precision arithmetic is used for double valued
%   inputs, all other types are computed using single precision. Output
%   image B is an array of the same size and type as A.
%
%   References
%   ---------
...
if ~isa(I,'double')
    % Compute in single precision (fast) unless inputs are double
    I = single(I);
end
s = settings;
if s.images.UseHalide.ActiveValue
    I = images.internal.builtins.diffusefilt_halide(I, gradientThreshold, N, connectivity, conductionMethod);
else
    if (dim == 2)
