...
classdef niqeModel
    ...
    methods
        function obj = niqeModel(Mean, Covariance, BlockSize, SharpnessThreshold)
            ...
            if nargin == 0
                ...
                matlab.images.internal.errorIfgpuArray(Mean, Covariance, BlockSize, SharpnessThreshold);
                validateMean(Mean);
    ...
    methods(Static, Hidden = true)
        ...
        function obj = computeNIQEModel(imds, BlockSize, SharpnessThreshold)
            ...
            obj = niqeModel(mean(full_NSSfeatures,1,'omitnan'),...
                cov(full_NSSfeatures,'omitrows'), BlockSize, SharpnessThreshold);
    ...
    methods(Hidden)
        ...
        function score = calculateNIQEscore(obj,im)
            ...
            mu_testparam     = mean(full_NSSfeatures,'omitnan');
            cov_testparam    = cov(full_NSSfeatures,'omitrows');
            covariance = obj.Covariance+cov_testparam;
            % Compute quality
            if(any(isnan(full_NSSfeatures)))
                warning(message('images:niqe:expectFiniteFeatures'));
                score = nan;
            elseif any(isnan(covariance))
                warning(message('images:niqe:solidImage'));
                score = nan;
            else
                invcov_param     = pinv((covariance)/2);
