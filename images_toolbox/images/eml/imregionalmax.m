...
if (coder.internal.preferMATLABHostCompiledLibraries() && ...
    ...
    BW = images.internal.coder.buildable.ImregionalmaxBuildable.imregionalmaxcore_logical(...
        ...
        ndims(connb),...
        size(connb));
elseif (coder.gpu.internal.isGpuEnabled && numel(size(connb)) == 2)
    %% GPU specific implementation
    % Do not consider floating point values with very similar values as
    % equal.
    floatEpsilonFlag = coder.const(false);
    BW = images.internal.coder.gpu.imregionalmaxAlgoImpl(I, connb, floatEpsilonFlag);
else
    % Portable C
