...
[B, combinedL] = findObjectBoundariesCodegen(BW, L, N, conn, findHoles);
...
% Branching Simulation and codegen version

if isempty(BW_in)
    BW = logical(BW_in);
elseif islogical(BW_in)
    BW = BW_in;
else
    [m,n] = size(BW_in);
    ...
    if coder.isColumnMajor
        parfor j=1:n
            for i=1:m
                BW(i,j) = BW_in(i,j)~=0; % handle NaN's as 1's
            end
        end
    else
        parfor i=1:m
...
%--------------------------------------------------------------------------
function [B, combinedL] = findObjectBoundariesCodegen(BW, L, N, conn, findHoles)
[m,n] = size(BW);
combinedL = coder.nullcopy(zeros(m,n));
if (findHoles)
    ...
    [labeledHoles,holesN] = bwlabel(BWholes, backgroundConn);

    if coder.isColumnMajor
        parfor j=1:n
            for i=1:m
                combinedL(i,j) = L(i,j) + (labeledHoles(i,j)~=0)*N + labeledHoles(i,j);
            end
        end
    else
        parfor i=1:m
    ...
    finder = images.internal.coder.BoundaryFinderObjectHoles(combinedL, conn);
    B = finder.findBoundaries(N, holesN);
else
    combinedL = L;
    % M code for C code generation
    finder = images.internal.coder.BoundaryFinder(combinedL, conn);
    B = finder.findBoundaries();
end

%--------------------------------------------------------------------------
function [B, L] = findHoleBoundariesSim(BW, conn)
