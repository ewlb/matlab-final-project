...
function [sigma, hsize, padding] = parseInputs(varargin)
    ...
    if nargin > 0 
        ...
        [filtSize, pad, filterDomain] = parseNameValuePairs(filterSizeDefault, paddingDefault, domainDefault, ...
                                                    varargin{beginNVIdx:end});
        hsize = validateFilterSize (filtSize);
        padding = validatePadding (pad);
        validateFilterDomain (filterDomain);
...
function [hsize, padding, filterDomain] = parseNameValuePairs(filterSizeDefault, paddingDefault, domainDefault,...
                                                   varargin)
    ...
    filterDomain = eml_get_parameter_value( ...
        optarg.FilterDomain, ...
        domainDefault, ...
        varargin{:});
end
...
% For codegen, only 'spatial' is supported for FilterDomain.
validatestring(filterDomain, {'auto','spatial'}, mfilename, 'FilterDomain');
