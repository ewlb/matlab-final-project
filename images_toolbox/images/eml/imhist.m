...
if coder.gpu.internal.isGpuImHistSupported()
    [yout, x] = gpucoder.internal.imhistImpl(a, n, top, offset, isMap);
else
    %--------------------------------------------------------------------------
    % Calculate histogram
    %--------------------------------------------------------------------------
    % The output is not initialized here but it must be initialized to zero
...
if (coder.internal.preferMATLABHostCompiledLibraries())
    ...
    switch(class(a))
        ...
        case 'single'
            if isScaled
                if coder.const(images.internal.coder.useOptimizedFunctions()) && ...
                        numel(a) > GRAIN_SIZE_PORTABLE && ~coder.internal.isInParallelRegion
                    [y, rngFlag, nanFlag] = imhistAlgo_floating_scaled_optimized(a,n,top,y);
                else
                    [y, rngFlag, nanFlag] = imhistAlgo_floating_scaled(a,n,top,y);
                end
            else
            if coder.const(images.internal.coder.useOptimizedFunctions()) && ...
                        numel(a) > GRAIN_SIZE_PORTABLE && ~coder.internal.isInParallelRegion
                    [y, rngFlag, nanFlag] = imhistAlgo_floating_optimized(a,n,y);
            else
                    [y, rngFlag, nanFlag] = imhistAlgo_floating(a,n,y);
                end
            end
...
%--------------------------------------------------------------------------
function [y, rngFlag, nanFlag] = imhistAlgo_floating_scaled_optimized(img,n,top,y)
...
temp = coder.nullcopy(coder.internal.indexInt(zeros(size(img))));
if coder.isColumnMajor() || (coder.isRowMajor() && numel(size(img))>3)
    parfor i = 1:numel(img)
        if ~isfinite(img(i))
            if isnan(img(i))
                nanFlag = nanFlag | true;
                idx = zeros(1,'like',img);
            else %inf
                idx =img(i);
            end
        else
        ...
        temp(i) = coder.internal.indexInt(idx);
    end
    for j = 1:numel(temp)
        idx = temp(j);
    ...
    if numel(size(img)) == 2
        ...
        parfor i = 1:size(img,1)
            for j = 1:imgSize2
                ...
                temp(i,j) = coder.internal.indexInt(idx);
            end
        end
        for i = 1:size(img,1)
            for j = 1:size(img,2)
        ...
        parfor i = 1:size(img,1)
            for j = 1:imgSize2
                for k = 1:imgSize3
                    ...
                    temp(i,j,k) = coder.internal.indexInt(idx);
                end
            end
        end
        for i = 1:size(img,1)
            ...
            for j = 1:size(img,2)
                for k = 1:size(img,3)
                    if isnan(img(i,j,k))
                        ...
                        % idx = floor(img(i,j,k) * scale + 0.5);
                        % floor() is not required as the subsequent call to
                        % coder.internal.indexPlus(idx(1),1) introduces a c-style cast of
                        % idx(1) before adding which truncates the index value as desired.
                        % idx is expected to be non-negative and less than the size of the
                        % output array before the cast is applied.
                        idx(1) = img(i,j,k) * scale + 0.5;
                    end
            ...
            for j = 1:size(img,2)
                for k = 1:size(img,3)
                    if isnan(img(i,j,k))
                        ...
                        value_minus_1 = cast(img(i,j,k) - 1.0,'double');
...
%--------------------------------------------------------------------------
function [y, rngFlag, nanFlag] = imhistAlgo_floating_optimized(img,n,y)
% Algorithm for floating-point, unscaled histogram binning.
...
if coder.isColumnMajor() || (coder.isRowMajor() && numel(size(img))>3)
    ...
    for p = 1:size(img,1)
        for q = 1:size(img,2)
                for r = 1:size(img,3)
                    idx = coder.internal.indexInt(double(img(p,q,r))*scale+0.5);
...
if coder.isColumnMajor() || (coder.isRowMajor() && numel(size(img))>3)
    ...
    for p = 1:size(img,1)
        for q = 1:size(img,2)
                for r = 1:size(img,3)
                    idx = coder.internal.indexInt(img(p,q,r));
