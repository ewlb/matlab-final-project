...
RR = coder.nullcopy(zeros(size(radiusRange,1),size(radiusRange,2)));
switch (objPolarity)
    case 'bright'
        RR = radiusRange;
    case 'dark'
...
w0 = coder.nullcopy(zeros(size(radiusRange,1),size(radiusRange,2)));
switch (method)
    case 'twostage'
...
for i = 1:xcStep:lenE
    ...
    for idx2 = 1:dim2
        for idx1 = 1:dim1
            xc(idx1,idx2) = round(Ex_chunk(idx1) + (-RR(idx2) * (Gx(idxE_chunk(idx1))/gradientImg(idxE_chunk(idx1)))));
            yc(idx1,idx2) = round(Ey_chunk(idx1) + (-RR(idx2) * (Gy(idxE_chunk(idx1))/gradientImg(idxE_chunk(idx1)))));
