...
if islogical(varargin{startIdx}) || isstruct(varargin{startIdx})
    ...
    if islogical(varargin{startIdx})
        ...
        if coder.gpu.internal.isGpuEnabled
            % GPU specific implementation is called for binary image(BW)
            % input case.
            [RegionIndices,RegionLengths,numObjs] = images.internal.coder.gpu.bwconncomp(varargin{startIdx}, 8);
            imageSize = size(varargin{startIdx});
            CC.Connectivity = 8;
        ...
        %REGIONPROPS(CC,...)
        if coder.gpu.internal.isGpuEnabled
            % GPU specific implementation is called for connected component
            % structure(CC) input case.
            CC = varargin{startIdx};
            validateCC(CC);

            % CC is a user input. It can be generated in SIM (for MEX
            % workflows) or Codegen. This SIM version does not contain
            % RegionIndices and RegionLengths fields. This has to be
            % handled in the code below. 
            if isfield(CC, "RegionIndices")
                RegionIndices = CC.RegionIndices;
                RegionLengths = CC.RegionLengths;
            else
                [RegionIndices, RegionLengths] = ...
                    images.internal.coder.pixIdxList2RegionInfo(CC.PixelIdxList);
            end
        else
            CC = varargin{startIdx};
            validateCC(CC);
    ...
    if coder.gpu.internal.isGpuEnabled
        % Unoptimized GPU code is generated for label image(L) input case.
        coder.internal.compileWarning('images:gpucodegen:RegionpropsUnsupportedSyntax');
    end
...
for k = coder.unroll(1 : numRequestedStats)
    switch requestedStats(k)
        ...
        case AREA
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputeAreaGPU(RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
        ...
        case FILLEDIMAGE
            if coder.gpu.internal.isGpuEnabled
                coder.internal.compileWarning('images:gpucodegen:RegionpropsUnsupportedProperties','FILLEDIMAGE');
            end
            [stats, statsAlreadyComputed] = ...
                ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
            [stats, statsAlreadyComputed] = ...
        ...
        case FILLEDAREA
            if coder.gpu.internal.isGpuEnabled
                coder.internal.compileWarning('images:gpucodegen:RegionpropsUnsupportedProperties','FILLEDAREA');
            end
            [stats, statsAlreadyComputed] = ...
                ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
            [stats, statsAlreadyComputed] = ...
        ...
        case CENTROID
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputeCentroidGPU(imageSize,RegionIndices,RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
                    ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
                [stats, statsAlreadyComputed] = ...
        ...
        case EULERNUMBER
            if coder.gpu.internal.isGpuEnabled
                coder.internal.compileWarning('images:gpucodegen:RegionpropsUnsupportedProperties','EULERNUMBER');
            end
            [stats, statsAlreadyComputed] = ...
                ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
            [stats, statsAlreadyComputed] = ...
        ...
        case EQUIVDIAMETER
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputeEquivDiameterGPU(RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
                    ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
                [stats, statsAlreadyComputed] = ...
        ...
        case EXTREMA
            if coder.gpu.internal.isGpuEnabled
                coder.internal.compileWarning('images:gpucodegen:RegionpropsUnsupportedProperties','EXTREMA');
            end
            [stats, statsAlreadyComputed] = ...
                ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
            [stats, statsAlreadyComputed] = ...
        ...
        case BOUNDINGBOX
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputeBoundingBoxGPU(imageSize,RegionIndices,RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
                    ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
                [stats, statsAlreadyComputed] = ...
        ...
        case SUBARRAYIDX
            if coder.gpu.internal.isGpuEnabled
                coder.internal.compileWarning('images:gpucodegen:RegionpropsUnsupportedProperties','SUBARRAYIDX');
            end
            [stats, statsAlreadyComputed] = ...
                ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
            [stats, statsAlreadyComputed] = ...
        ...
        case MAJORAXISLENGTH
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputeEllipseParamsGPU(imageSize,RegionIndices,RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
                    ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
                [stats, statsAlreadyComputed] = ...
        ...
        case EXTENT
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputeExtentGPU(imageSize,RegionIndices,RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
                    ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
                [stats, statsAlreadyComputed] = ...
        ...
        case IMAGE
            if coder.gpu.internal.isGpuEnabled
                coder.internal.compileWarning('images:gpucodegen:RegionpropsUnsupportedProperties','IMAGE');
            end
            [stats, statsAlreadyComputed] = ...
                ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
            [stats, statsAlreadyComputed] = ...
        ...
        case PIXELLIST
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputePixelListGPU(imageSize,RegionIndices,RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
                    ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
                [stats, statsAlreadyComputed] = ...
        ...
        case PERIMETER
            ...
            if coder.gpu.internal.isGpuEnabled
                coder.internal.compileWarning('images:gpucodegen:RegionpropsUnsupportedProperties','PERIMETER');
            end
            [stats, statsAlreadyComputed] = ...
                ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
            if ~isempty(L)
        ...
        case PIXELVALUES
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputePixelValuesGPU(I,imageSize,RegionIndices,RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
                    ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
                [stats, statsAlreadyComputed] = ...
        ...
        case WEIGHTEDCENTROID
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputeWeightedCentroidGPU(I,imageSize,RegionIndices,RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
                    ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
                [stats, statsAlreadyComputed] = ...
        ...
        case MEANINTENSITY
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputeMeanIntensityGPU(I,RegionIndices,RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
                    ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
                [stats, statsAlreadyComputed] = ...
        ...
        case MININTENSITY
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputeMinIntensityGPU(I,RegionIndices,RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
                    ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
                [stats, statsAlreadyComputed] = ...
        ...
        case MAXINTENSITY
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputeMaxIntensityGPU(I,RegionIndices,RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
                    ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
                [stats, statsAlreadyComputed] = ...
        ...
        case CIRCULARITY
            if coder.gpu.internal.isGpuEnabled
                coder.internal.compileWarning('images:gpucodegen:RegionpropsUnsupportedProperties','CIRCULARITY');
            end
            [stats, statsAlreadyComputed] = ...
                ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
            [stats, statsAlreadyComputed] = ...
                ComputeCircularity(L, imageSize, numObjs, stats, statsAlreadyComputed);

        case PIXELIDXLIST
            if coder.gpu.internal.isGpuEnabled && (islogical(varargin{startIdx}) || isstruct(varargin{startIdx}))
                [stats, statsAlreadyComputed] = ...
                    images.internal.coder.gpu.regionpropsGPUImpl.ComputePixelIdxListGPU(RegionIndices,RegionLengths,numObjs,stats,statsAlreadyComputed);
            else
                [stats, statsAlreadyComputed] = ...
                    ComputePixelIdxList(L, CC, numObjs, stats, statsAlreadyComputed);
            end
...
if ~statsAlreadyComputed.PixelIdxList
    ...
    if numObjs ~= 0
        ...
        if isempty(CC) || isfield(CC, "RegionIndices")
            % Use this code path if the input to CC is a label matrix OR if
            % a Codegen CC
            idxCount = coder.internal.indexInt([0;cumsum(regionLengths)]);
            for k = 1:numel(stats)
                if coder.isColumnMajor
            ...
            % This code path is hit only if the user input a SIM CC struct,
            % use the makeHomogeneous function to specify this.
            % The indexing below works only if PixelIdxList is declared to
            % be homogeneous at compile time. 
            for k = 1:numel(stats)
                if coder.isColumnMajor
                    stats(k).PixelIdxList = CC.PixelIdxList{k};
                else
                    % Sort the indices to match column-major output
                    stats(k).PixelIdxList = sort(CC.PixelIdxList{k});
                end
            end
        end
...
if ~statsAlreadyComputed.Circularity
    ...
    for k = 1:length(stats)
            ...
            % Normal formula for circularity.
            c = 4*pi*a ./ p^2;
            
            % Because of the way the perimeter is measured, which is by
            % tracing pixel centers, the perimeter measurement is based
            % roughly on a "radius" (for circles) that is 0.5 less than the
            % effective radius for the area measurement. This causes the
            % result from the above formula to be biased high. The
            % following scale factor is used to correct for the bias.
            %
            % This bias correction is an unpublished method, Steve Eddins,
            % MathWorks, 2022. See notes attached to MathWorks record
            % g2213383.
            r_eq = p/(2*pi) + 0.5;
            correction = (1 - (0.5/r_eq))^2;

            % Because of the approximate nature of the perimeter
            % approximation, some computed values may still be slightly
            % greater than 1 even after the correction factor is applied.
            % Use the min() function to force the circularity measurements
            % to be no greater than 1.
            stats(k).Circularity = min(c * correction, 1);
    end
end
end
%--------------------------------------------------------------------------
...
if numObjs > 0
    ...
    for c = 2:numPadCols-1
        for r = 2:numPadRows-1
            ...
            % label.
            if  pixelLabel > 0
                if L(labelR,labelC) > 0 && L(labelR,labelC) ~= currentLabel && visitedLabels(pixelLabel) == false
...
shapeMeasurementProperties = { ...
    ...
    'Perimeter', 'PixelIdxList', 'PixelList', 'Circularity'};

unsupportedProperties = { ...
    'ConvexArea', 'ConvexHull', 'ConvexImage', 'MaxFeretProperties', 'MinFeretProperties', 'Solidity', 'SubarrayIdx'};

% Concatenate official and pixel value statistics.
officialAndPixelValueStatsStrs = { ...
    pixelValueStatsStrs{:} shapeMeasurementProperties{:} unsupportedProperties{:}};

p = 1;
for k = coder.unroll(startIdx:numel(varargin))
    coder.internal.errorIf(~(ischar(varargin{k}) || isstring(varargin{k})),'images:regionprops:invalidType');
    ...
    elseif strcmp(prop,'Circularity')
        statsOneObj.Circularity = 0;
...
for fIdx = coder.unroll(0:eml_numfields(CC)-1)
    ...
    tf = tf && any(strcmp(fieldName,{'Connectivity','ImageSize','NumObjects','RegionLengths','RegionIndices','PixelIdxList'}));
end
