...
switch nargin
    case 1
        if coder.gpu.internal.isGpuEnabled && (numel(size(I))==2)
            % Optimized GPU implementation requires a 2D input and
            % two-dimensional connectivity
            conn = 4; % Use default connectivity
        else
...
useSharedLibrary = coder.internal.preferPrecompiledLibraries();

if isempty(I)
    I2 = I;
elseif do_fillholes
    % for 2D BW image, connFlag could be 4/8 depending on connectivity
    % for 3D BW image, connFlag could be 6/18/26 depending on connectivity
    % modeFlag will be 0 if custom connectivity matrix is used
    connFlag = images.internal.coder.bwfloodfillGetConnectivity(conn, I);
    if ~useSharedLibrary && islogical(I) && connFlag~=0
        I = imcomplement(I);
        if ismatrix(I) % 2D BW image
            img = padarray(I,[1 1], true);
            res = images.internal.coder.bwfloodfill2d(img,connFlag,"fillHole");
            I2 = res(2:end-1,2:end-1);
        else % 3D BW image
            img = padarray(I,[1 1 1], true);
            res = images.internal.coder.bwfloodfill3d(img,connFlag,"fillHole");
            I2 = res(2:end-1,2:end-1,2:end-1);
        end
        I2 = imcomplement(I2);
        return;
    elseif islogical(I)
    ...
    if ~useSharedLibrary && islogical(I) && connFlag~=0
        if ismatrix(I)
            I2 = images.internal.coder.bwfloodfill2d(mask,connFlag,"seedIndex",locationsVar);
        else
            I2 = images.internal.coder.bwfloodfill3d(mask,connFlag,"seedIndex",locationsVar);
        end
        I2 = imcomplement(I2);
        return;
    end
