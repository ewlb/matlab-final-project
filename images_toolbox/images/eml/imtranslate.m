...
if isscalar(fillValues)
    ...
    if (coder.isColumnMajor)
        ...
        numRow = coder.internal.indexInt(outputSize(1));
        numCol = coder.internal.indexInt(outputSize(2));
        lenFillValues = coder.internal.indexInt(length(fillValues));
        parfor rowIdx = 1:numRow
            for colIdx = 1:numCol
                for i = 1:lenFillValues
                    out(rowIdx,colIdx,i) = fillValueCastToOutputType(i); 
                end
            end
        end
    end
...
if locationsInSourceMapToDestination
    ...
    % Handle Row Major case for RGB images separately
    
    if numPlanes<2 || coder.isColumnMajor
        for i = 1:numPlanes
        ...
        outputColStart = coder.internal.indexInt(outputC(1));
        outputColEnd = coder.internal.indexInt(outputC(2));
        outputRowStart = coder.internal.indexInt(outputR(1));
        outputRowEnd = coder.internal.indexInt(outputR(2));

        UWorldClippedStart = coder.internal.indexInt(UWorldClippedToBounds(1));
        VWorldClippedStart = coder.internal.indexInt(VWorldClippedToBounds(1));

        parfor rowIdx = outputRowStart:outputRowEnd
            VWorldClippedRow = VWorldClippedStart + rowIdx - outputRowStart;
            for colIdx = outputColStart:outputColEnd
                UWorldClippedCol = UWorldClippedStart + colIdx - outputColStart;
                for i=1:numPlanes
                    out(rowIdx,colIdx,i) = A(VWorldClippedRow,UWorldClippedCol,i); 
                end
            end
        end
    end
