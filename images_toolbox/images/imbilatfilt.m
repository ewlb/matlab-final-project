...
%     % Compute in single precision instead of double to improve
%     % performance.
%     ims = im2single(im);
%     iml = rgb2lab(ims);
%
...
if s.images.UseHalide.ActiveValue
    ...
    computeInDouble = ~( isa(opts.A,'single') || isa(opts.A,'uint8'));

    if computeInDouble
        spatialWeights = []; % unused in this code path
        opts.A = double(opts.A);
    else % compute in single precision
        spatialWeights = fspecial('gaussian', opts.NeighborhoodSize, opts.SpatialSigma);
        spatialWeights = single(spatialWeights);
    end

    B = images.internal.builtins.bilatfilt_halide(opts.A, opts.NeighborhoodSize,...
        rangeSigma, spatialWeights, opts.Padding, opts.PadVal, opts.SpatialSigma);
