...
pad_val = cast(getPadValue(A,op_type),underlyingType(A));
...
for k = 1 : num_strels
        mask{k} = gpuArray(cast(getnhood(se(k)),underlyingType(A))); 
end
...
if unpad
    % Note: This went through the padfull codepath. We crop accordingly.
    ...
    for k = 1:2
        ...
        idx{k} = matlab.internal.ColonDescriptor(first, last);
    end
    for k = 3:ndims_A
        idx{k} = ':';
    end
