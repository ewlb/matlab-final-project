...
switch opStr
    ...
    case 'spur'
        ...
        ff = xor(bw(1:2:end, 1:2:end), endPoints(1:2:end, 1:2:end));
        bw(1:2:end, 1:2:end) = ff;        
        ...
        ff = xor(bw(1:2:end, 2:2:end), newEndPoints(1:2:end, 2:2:end));
        bw(1:2:end, 2:2:end) = ff;
        
        % In the third field, remove any of the original end points
        % that are still end points.
        newEndPoints = endPoints & images.internal.gpu.bwlookup(bw, gpuArray(lut));
        ff = xor(bw(2:2:end, 1:2:end), newEndPoints(2:2:end, 1:2:end));
        bw(2:2:end, 1:2:end) = ff;
        
        % In the fourth field, remove any of the original end points
        % that are still end points.
        newEndPoints = endPoints & images.internal.gpu.bwlookup(bw, gpuArray(lut));
        ff = xor(bw(2:2:end, 2:2:end), newEndPoints(2:2:end, 2:2:end));
        bw(2:2:end, 2:2:end) = ff;
