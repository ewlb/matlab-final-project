...
if isfield(parsedInputs,'GeometricTransform')
    T = parsedInputs.GeometricTransform;
    % We only allow displacement fields, projective, and affine and its special
    % cases as white-listed transformations.
    if ~isnumeric(T) && ~isa(T,'projective2d') && ~isa(T,'affine2d') && ~isa(T,'affine3d') && ~isa(T,'rigid2d') && ~isa(T,'rigid3d') &&...
            ~isa(T,'affinetform2d') && ~isa(T,'affinetform3d') && ~isa(T,'projtform2d') && ~isa(T,'transltform2d') && ~isa(T,'simtform2d') &&...
            ~isa(T,'transltform3d') && ~isa(T,'simtform3d') && ~isa(T,'rigidtform2d') && ~isa(T,'rigidtform3d')

        error(message('images:imwarp:gpuUnsupportedTransformationType'));
    end
end
...
if isa(tform,'affine3d') || ndims(tform)==4
...
switch(method)
    ...
    case 'linear'
        B = arrayfun(@warpElemLinear, x, y, z);
    case 'nearest'
        B = arrayfun(@warpElemNearest, x, y, z);
    otherwise
        error('');
end
...
B = cast(B,origClass);
    ...
    function vOut = interp3dLinear(u,v,w)
        ...
        if outOfBounds(u,v,w)
            ...
            valueWest = weightNorth*in(vNorth,uWest,wUp) + weightSouth*in(vSouth,uWest,wUp);
            valueEast = weightNorth*in(vNorth,uEast,wUp) + weightSouth*in(vSouth,uEast,wUp);
            
            v1 = weightWest*valueWest + weightEast*valueEast;
            
            valueWest = weightNorth*in(vNorth,uWest,wDown) + weightSouth*in(vSouth,uWest,wDown);
            valueEast = weightNorth*in(vNorth,uEast,wDown) + weightSouth*in(vSouth,uEast,wDown);
    ...
    function vOut = interp3dNearest(u,v,w)
        
        if outOfBounds(u,v,w)
            vOut = fillValue;
        else
            vOut = in(round(v),round(u),round(w));            
        end
    ...
    function vOut = warpElemDisplacementNearest(xLoc,yLoc,zLoc)
        ...
        [u,v,w] = mapPointDisplacement(xLoc,yLoc,zLoc);    
        vOut = interp3dNearest(u,v,w);
        
    end
    
    function vOut = warpElemLinear(xLoc,yLoc,zLoc)
    ...
    function vOut = warpElemNearest(xLoc,yLoc,zLoc)
