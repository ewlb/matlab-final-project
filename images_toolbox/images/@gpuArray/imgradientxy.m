...
switch method
    ...
    case 'centraldifference' 
        if isrow(I)            
            Gx = gradient2(I);
            if nargout > 1
                Gy = gpuArray.zeros(size(I),underlyingType(I));
            end            
        elseif iscolumn(I)            
            Gx = gpuArray.zeros(size(I),underlyingType(I));
            if nargout > 1
                Gy = gradient2(I);
            end                
        else            
...
out = gpuArray.zeros([m,n],underlyingType(in));
...
if nargout == 2
    out2 = gpuArray.zeros([m,n],underlyingType(in));
