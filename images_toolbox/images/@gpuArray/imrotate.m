    ...
    switch (r)
        ...
        case {90, 270, -90, -270}
            ...
            if strcmpi(bbox, 'crop') && not_square
                ...
                % pre-allocate array
                B = gpuArray.zeros(new_size,underlyingType(A));

                % Fill the central region with the rotated result
                % ROT90 can rotate all pages at once
                B(v{:}) = rot90(A(v{:}), multiple_of_ninety);
            else
        ...
        case {180,-180}
            ...
            v = repmat({':'},[1 ndims(A)]);
            v{1} = matlab.internal.ColonDescriptor(twod_size(1), -1, 1);
            v{2} = matlab.internal.ColonDescriptor(twod_size(2), -1, 1);
...
% validate image
A = varargin{1};
validateattributes(A,{'uint8','uint16','logical','single'},{'nonsparse'},mfilename,'input image',1);
