...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [img,classImg,imageType,low_in,high_in,low_out,high_out,gamma] = ...
        ...
        classImg = underlyingType(img);
        
        if nargin == 1
            % IMADJUST(I)
            if ~ismatrix(img)
                if (ndims(img)==3 && size(img,3)==3)
                    error(message('images:imadjust:oneArgOnlyGrayscale'))
                else
                    error(message('images:imadjust:invalidImage'))
                end
            end
            ...
            checkRange(lowhigh_in, imageType,'[LOW_IN; HIGH_IN]');
            checkRange(lowhigh_out, imageType,'[LOW_OUT; HIGH_OUT]');
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function checkRange(range, imageType, variableName)
        
        if strcmp(imageType, 'intensity')
            if numel(range) ~= 2
                error(message('images:imadjust:InputMustBe2ElVec', variableName))
            end
        else
            if ~(numel(range) == 2 || isequal(size(range), [2 3]))
                error(message('images:imadjust:InputMustBe2ElVecOr2by3Matrix', variableName));
            end
        end
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
