...
% dispatch to CPU.
if ~isgpuarray(argin{1})
    [argin{:}] = gather(argin{:});
    outstats = regionprops(argin{:});
    return;
end
...
if islogical(L)
    ...
    numObjs = max( 0, floor(double(gather(max(L,[],"all")))) );
end
...
if ~statsAlreadyComputed.Extrema
    ...
    for k = 1:length(stats)
        ...
        if isempty(pixelList)
            ...
            [minR,maxR,minC,maxC] = gather(min(r),max(r),min(c),max(c));
            ...
            % bottom-row pixels.
            tmp = c(maxRSet);
            [c5,c6] = gather(max(tmp),min(tmp));

            % Points 7 and 8 are on the left column.
            % Find the minimum and maximum row coordinates for
            % left-column pixels.
            tmp = r(minCSet);
            [r7,r8] = gather(max(tmp),min(tmp));
            c7 = minC;
            c8 = minC;
    ...
    for k = 1:length(stats)
        ...
        if any(cellfun(@isempty,subarrayIdx))
            ...
            if min(sizeOfSubImage) == 0
                ...
                cc = stats(k).PixelList(:,1);
                rr = stats(k).PixelList(:,2);
...
if ~statsAlreadyComputed.PixelValues
    ...
    for k = 1:length(stats)
        % reshape: force column vector if row vector image
         stats(k).PixelValues = reshape(I(stats(k).PixelIdxList), [], 1);
    end
end
...
if ~statsAlreadyComputed.WeightedCentroid
    ...
    for k = 1:length(stats)
        ...
        for n = 1:2
            M = gather(sum(stats(k).PixelList(:,n) .* double(stats(k).PixelValues)));
            wc(n) = M ./ sumIntensity;
        end
