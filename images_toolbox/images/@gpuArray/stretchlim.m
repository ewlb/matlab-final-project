...
if  ~isgpuarray(varargin{1}) || isUnderlyingType(varargin{1},'uint8')
    % Dispatch to CPU 
    [varargin{:}] = gather(varargin{:});
    lowhigh = gpuArray(stretchlim(varargin{:}));
    return;
end

[img,tol] = ParseInputs(varargin{:});

if isUnderlyingType(img,'uint8')
    nbins = 256;
else
    nbins = 65536;
end
...
if tol_low < tol_high
    ...
    for i = 1:p                          % Find limits, one plane at a time
        N     = images.internal.gpu.imhist(img(:,:,i),nbins);
...
{'real','nonempty','nonsparse'},mfilename, 'I or RGB', 1);
