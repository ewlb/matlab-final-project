...
if mod(nargin,2) ~= 0
    error(message('images:makeresampler:invalidNumInputs'));
end
npairs = nargin / 2;
...
if isempty(type)
    error(message('images:makeresampler:missingTYPE'));
end
switch GetCanonicalString(type,'Type',{'separable','custom'})
  ...
  otherwise
    error(message('images:makeresampler:internalError'))
end
...
if isempty(n_dimensions)
    error(message('images:makeresampler:unspecifiedNDims'))
end
if length(n_dimensions) ~= 1
    error(message('images:makeresampler:invalidNDimsNotScalar'))
end
if ~isa(n_dimensions,'double') || ~isreal(n_dimensions)
    error(message('images:makeresampler:invalidNDimsNotReal'))
end
if n_dimensions ~= floor(n_dimensions) || n_dimensions < 1
    error(message('images:makeresampler:invalidNDimsNotPositive'))
end

resample_fcn = FindValue('resamplefcn', property_strings, varargin{:});

if isempty(resample_fcn)
    error(message('images:makeresampler:resampleFcnNotSpecified'))
end

if length(resample_fcn) ~= 1
    error(message('images:makeresampler:invalidResampleFcnScalar'))
end
if ~isa(resample_fcn,'function_handle')
    error(message('images:makeresampler:invalidResampleFcnHandle'))
end
...
r = AssignResampler(n_dimensions, padmethod, @(varargin) images.internal.builtins.resampsep(varargin{:}), rdata);
...
switch interpolant
 ...
 otherwise
    error(message('images:makeresampler:invalidInterpolant'))
end
...
if length(kernel) ~= 1 || ~isa(kernel,'function_handle')
   error(message('images:makeresampler:invalidKernel'))
end
...
for i = 1:((nargin-2)/2)
    ...
    if ischar(current_name)
        ...
        if nmatch > 1
            error(message('images:makeresampler:ambiguousPropertyName', current_name))
        end
        if nmatch == 1
            ...
            if strcmp(canonical_name, property_name)
                if isempty(value)
                    if isempty(varargin{2*i})
                        error(message('images:makeresampler:emptyPropertyName', property_name))
                    end
                    value = varargin{2*i};
                else
                    error(message('images:makeresampler:redundantPropertyName', property_name))
                end
            end
        end
    end
...
if ~ischar(input_string)
    error(message('images:makeresampler:invalidPropertyName', property_name))
end
...
if nmatch == 0
    error(message('images:makeresampler:unknownPropertyName', property_name, input_string))
end

if nmatch > 1
    error(message('images:makeresampler:ambiguousPropertyValue', property_name, input_string))
end
...
for i = 1:((nargin-2)/2)
    ...
    if ischar(current_name)
        ...
        if nmatch == 0
            warning(message('images:makeresampler:ignoredPropertyName', current_name, type))
        end
        if nmatch > 1
            error(message('images:makeresampler:ambiguousPropertyName', current_name))
        end
    else
        error(message('images:makeresampler:invalidPropertyName', current_name))
    end
end
