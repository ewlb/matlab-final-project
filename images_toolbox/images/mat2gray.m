function I = mat2gray(A,limits)%#codegen
...
if nargin == 1
    % GPU enabled condition is added here. Rewrote the code using 
    % gpucoder.reduce pragma to generate optmized GPU code.
    if coder.gpu.internal.isGpuEnabled
        limits = double(gpucoder.reduce(A,{@funcMin,@funcMax}));
    else
...
function c = funcMax(a, b)
c = max(a,b);

% Function call for 'min'
function c = funcMin(a, b)
c = min(a,b);