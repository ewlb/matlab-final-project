...
if isTableRequested
    ...
    varNames = outstats.Properties.VariableNames;

    if ismember('PixelIdxList', varNames)
        if (~ iscell(outstats.PixelIdxList) && ~isempty(outstats.PixelIdxList))
              % Convert pixelIdList to a cell array
              outstats.PixelIdxList = convertNumericToCell(outstats.PixelIdxList);
        end
    end
    
    % These three properties produce outputs whose dimensions are dependent
    % upon the input image. This can result in the table variables either
    % be a numeric array or a cell array that holds this numeric array.
    % This mismatch makes it hard to work with the output tables. Ensuring
    % these are consistent.
    if ismember("Image", varNames)
        if ~iscell(outstats.Image)
            outstats.Image = convertNumericToCell(outstats.Image);
        end
    end

    if ismember("ConvexImage", varNames)
        if ~iscell(outstats.ConvexImage)
            outstats.ConvexImage = convertNumericToCell(outstats.ConvexImage);
        end
    end
    if ismember("FilledImage", varNames)
        if ~iscell(outstats.FilledImage)
            outstats.FilledImage = convertNumericToCell(outstats.FilledImage);  
        end
    end
end

function out = convertNumericToCell(in)
    rowSizes = ones(1, size(in, 1));
    columnSize = size(in, 2);
      
    out = mat2cell(in, rowSizes, columnSize);
...
if(size(delta,1) > 1)
    % References:
    % Vossepeol and Smeulders, "Vector Code Probability and Metrication
    % Error in the Representation of Straight Lines of Finite Length,"
    % Computer Graphics and Image Processing, vol. 20, pp. 347-364, 1982.
    %
    % Luego, "Measuring boundary length," Cris' Image Analysis Blog:
    % Theory, Methods, Algorithms, Applications, 14-Sep-2010, accessed
    % 24-Sep-2011. Current URL, as of 19-Aug-2022: 
