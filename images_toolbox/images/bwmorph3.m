...
%   Examples
%   --------
%   % This example carries out the 'clean' and 'majority' morphological operations
...
matlab.images.internal.errorIfgpuArray(V,operation);
[bwin,op] = parseInputs(V,operation);

s = settings;
if ndims(bwin) == 3 && s.images.UseHalide.ActiveValue
    out = images.internal.builtins.bwmorph3_halide(bwin, ['h' op]);
else
