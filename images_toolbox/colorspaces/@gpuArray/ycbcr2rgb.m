...
if strcmp(classIn,'uint8')
    ...
    for p = 1:3
        ...
        if strcmp(classIn,'uint16')
            ...
            % for uint16 input
            rgb(:,:,p) = arrayfun(@lincombuint16,y,cb,cr,TY,TCb,TCr,offsetp);
        else
            % Use private function lincomb to calculate RGB output
            rgb(:,:,p) = arrayfun(@lincomb,y,cb,cr,TY,TCb,TCr,offsetp);
        end
    end
...
if ismatrix(X)
    validateattributes(X,{'double','single'},{'real','nonempty','nonsparse'},mfilename,'MAP',1);
...
elseif ndims(X) == 3
    validateattributes(X,{'uint8','uint16','double','single'},{'real','nonsparse'},mfilename,'YCBCR',1);
