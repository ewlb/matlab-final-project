...
%   RGB can be: 
%   1. P-by-3 gpuArray of colors (one color per line) OR
%   2. M-by-N-by-3 gpuArry representing an image
...
%
%   BPS is a numeric scalar or gpuArray containing numeric scalar that
%   specifies the actual number of bits required to represent each channel
%   of the input. It can take values 10 or 12.
%
%   Class Support
%   -------------
%   RGB must contain uint16 data. The output YCBCR will be a gpuArray
%   containing uint16 data.
%
%   Notes
%   -----
...
%
%      RGBLIST = gpuArray(uint16([940 940 940]));
...
%      XYZ = rgb2xyz(IM);
%      RGB = gpuArray(xyz2rgbwide(XYZ, 12));
%
%      % Convert to YCBCR
%      YCBCR = rgbwide2ycbcr(RGB, gpuArray(12));
%
%   References:
%   ----------
...
%       PROGRAMME EXCHANGE"
%
%   See also GPUARRAY/YCBCR2RGBWIDE, GPUARRRAY/RGB2YCBCR, GPUARRAY/YCBCR2RGB.
...
%   Copyright 2020, MathWorks Inc.
    ...
    arguments
        ...
        bitsPerSample (1, 1) { mustBeMember(bitsPerSample, [10, 12]) }
    end
...
%   RGB can be: 
%   1. P-by-3 gpuArray of colors (one color per line) OR
%   2. M-by-N-by-3 gpuArray representing an image
