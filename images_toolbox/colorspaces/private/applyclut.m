...
% All of the processing steps within applyclut operate on data within
% a [0,1] range, independent of the precision of the lookup table,
% which can contain either uint8 or uint16 values.
%
...
if ~isempty(luttag.CLUT)
    ...
    % Convert the input lookup table to double, scaling the values to be 
    % in the range [0,1].
    clut = im2double(luttag.CLUT);
    
    switch chan_in
      case 3
        out = clutinterp_tet3(out, clut, chan_in, chan_out);
      case 4
        out = clutinterp_tet4(out, clut, chan_in, chan_out);
      otherwise
        out = clutinterp(out, clut, chan_in, chan_out);
    end
...
% Re-encode the output to match the input encoding, which can be either
% uint16 or uint8.
if isa(in,'uint16')
    out = im2uint16(out);
else
...
%
% Input and output color values are in the range [0,1].
%
% The lookup table, clut, should be floating-point in the range [0,1].
...
for k = 1 : chan_in
    samples{k} = linspace(0, 1, size(clut, k));
end
...
res = wgts(wgt_inds) .* clut2(base_inds,:);
for i = 2 : 4
    inds = base_inds + off_ids(ids, i);
    wgt_inds = (1 : n)' + wgt_ids(ids, i) * n;
    res = res + wgts(wgt_inds) .* clut2(inds,:);
end
%--------------------------------------------------------------------------
