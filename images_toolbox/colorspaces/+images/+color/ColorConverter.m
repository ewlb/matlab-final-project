classdef (Sealed) ColorConverter < images.color.internal.Callable & matlab.mixin.CustomDisplay
    ...
    methods
        ...
        function out = convert(self,in,varargin)
            % convert Convert colors
            %
            %    out = convert(converter,in,varargin)
            %
            %    out = convert(converter,in,varargin) uses the color converter to convert colors. The input
            ...
            for k = 1:P
                ...
                for b = 1:num_blocks
                    ...
                    v = in(first:last,:,k);
                    v = self.InputEncoder.decode(v, varargin{:});
                    v = self.convertUnencoded(v);
                    v = self.OutputEncoder.encode(v,output_type,varargin{:});
                    out(first:last,:,k) = v;
                end
            end
    ...
    methods (Hidden)
        function out = evaluate(self, in, varargin)
            % evaluate Convert colors
            %
            %    out = evaluate(converter,in, varargin)
            %
    ...
    methods (Access = protected)
        ...
        function s = getFooter(self)
            ...
            if feature('hotlinks')
                ...
                s = sprintf('<a href="matlab:web([''text://%s''])">%s</a>\n',t4,...
