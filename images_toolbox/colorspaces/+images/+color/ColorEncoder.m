classdef ColorEncoder
    ...
    methods
        function out = decode(this_encoder, in, varargin)
            ...
            if nargin(decoder_function) > 1
                out = decoder_function(in, varargin{:});
            else
                out = decoder_function(in);
            end
        end
        
        function out = encode(this_encoder, in, output_type, varargin)
            ...
            if nargin(encoder_function) > 1
                out = encoder_function(in, varargin{:});
            else
